...
| 中位数 CSV | `01_quantizer_fair/<dataset>/csv/formal_test_median.csv` | 5 次 repeat 的统计结果，主表优先使用 |
...
| 中位数 CSV | `02_diskann_fair/<dataset>/csv/formal_test_median.csv` | 5 次 repeat 的统计结果，主表优先使用 |
...
| 中位数 CSV | `03_system_fair/<dataset>/csv/formal_test_median.csv` | 5 次 repeat 的统计结果，主表优先使用 |
...
此外，每个 operating point 的 `repeat_id` 必须严格等于 `{0,1,2,3,4}`，`formal_test_median.csv` 必须由这五次原始 artifact 生成并写入 `repeat_id=median_of_5`。不得用复制、拼接或单次结果生成名为 median 的文件。

每个指标除 median 外还应报告 IQR 或 bootstrap 95% CI。CV 超过 5% 用于触发硬件状态排查，不得通过反复重跑并只保留“稳定”批次来消除波动。MSMARCO 当前只有 1,677 个 query；在 p99 进入主表前，必须扩充独立 test queries 或将 validation 规模降低到能保留足够 test queries。

不满足上述条件的结果仅作为诊断结果，不进入正式报告主表或主图。当前 `results/disk_environment` 中仅含 `repeat_id=0` 或无法追溯原始 artifact 的发布 CSV 均视为 legacy/diagnostic，必须从同一 immutable run ID 重新运行和聚合后才能升格为 formal。
