...
当前 `/home/kai3/coco` 位于旋转盘 RAID，因此只能用于开发、转换和 smoke test。正式运行必须通过：

```text
--disk-root /mnt/exclusive_nvme/qgraph
```

显式指定独占 NVMe 路径；若预检失败，程序必须拒绝生成正式结果。
...
- 三数据集 fixed-candidate Recall@10–QPS，`resident` 和 `payload_on_ssd` 分面或使用清楚的线型区分。
- 三数据集 p95 latency 分解：I/O wait、decode/distance compute、query preparation。
- matched fixed-candidate recall 下的 code bytes/vector、4 KiB pages/query 和 read amplification。
- 距离误差图继续报告 mean/P95 relative error 和 pairwise flip rate，证明 storage mode 不改变数值质量。

### 13.2 05B 实验 02 磁盘图与 Ours 机制图

- 三数据集 Recall@10–p95 latency 和 Recall@10–QPS。
- matched recall 下比较 PQ、SQ、SAQ、Ours-Disk 的 adjacency pages、payload pages 和 total bytes/query；图注明确 Ours 保留自己的 ExRaBitQ-symmetric 图。
- Ours 四级消融使用瀑布图或配对点图展示 DB1 gate、page coalescing 和 page reuse 分别减少多少 I/O 与延迟。
- 图注声明 PQ/SQ/SAQ 共享一个 graph SHA-256，Ours 在 05B/05C 使用同一个 native Ours graph SHA-256；不得声称四者图相同。

### 13.3 05C Recall–latency

- 三个横向面板：AGNews、GIST、DBpedia。
- x 轴：Recall@10。
- y 轴：p95 latency，建议 log scale。
- 方法为 Ours、SymphonyQG、OG-LVQ、Glass-NSG 和 DiskANN-PQ。
- 只绘制实测 Pareto 点。

### 13.4 05C Recall–throughput

- 三个横向面板，x 轴 Recall@10，y 轴 16-worker aggregate QPS。
- GIST 的 workers=`1/4/8/16/32` 并发扩展曲线放入补充图。

### 13.5 I/O 原因图、内存敏感性与资源表

在 matched recall 下报告：

- 4 KiB sectors/query
- bytes/query
- I/O requests/query
- cache hit rate
- Ours 的 DB1 survivor ratio

GIST 绘制：

- `B=1/2/4 GiB` 的 Recall–p95 latency
- `B=1/2/4 GiB` 的 pages/query
- `C=0` 与标准 BFS cache 的差异
