...
def load_diagnostic(relative):
    """Load a complete row set while preserving formal_ready=false diagnostics."""
    root = ROOT / relative
    table = root / 'tables/formal_test_rows.csv'
    manifest = root / 'tables/formal_test_rows.manifest.json'
...
def main():
    global TAG,OUT,BEAM,BUDGETS_05
    ...
    parser.add_argument('--beam',type=int,default=int(os.environ.get('QG05_FIXED_BEAM','1')))
    parser.add_argument('--budgets',default=os.environ.get('QG05_BUDGETS','1,2,4,8'))
    args=parser.parse_args()
    TAG=args.tag
    BEAM=args.beam
    BUDGETS_05=[float(x) if '.' in x else int(x) for x in args.budgets.split(',') if x]
    OUT=ROOT/'results/diagnostics'/TAG
    ...
    state={'status':'running','tag':TAG,'datasets':datasets,'order':'dataset: 03 then 05',
           ...
           'widths':[10,20,40,60,100,160,240,400,580], 'beam':BEAM,'budgets_05':BUDGETS_05,
           'methods_03':METHODS,'methods_05':['Ours-Disk'],'jobs':[],
           'protocol':f'uniform beam={BEAM}; validation hot profile only; native/internal reference and RSS admission; external parity skipped by user request'}
    ...
    save()
    env=dict(os.environ, PATH=str(ROOT/'work/tools/numactl/root/usr/bin')+os.pathsep+os.environ['PATH'], QG05_SKIP_EXTERNAL_PARITY='1')
    ...
    try:
        for dataset in state['datasets']:
            for experiment in experiments:
                sources=[]
                for budget in ([4] if experiment.startswith('03') else BUDGETS_05):
                    run_id=f'{dataset}_{TAG}_{experiment[:2]}_ram{budget}'
                    previous=next((j for j in state['jobs'] if j['run_id']==run_id),None)
                    if previous and previous['status']=='completed':
                        sources.append(load_diagnostic(f'results/{experiment}/{dataset}/{run_id}'))
                        job=previous
                        continue
                    ...
                    cmd=[sys.executable,str(ROOT/'experiments'/experiment/'run.py'),'--datasets',dataset,
                         ...
                         '--numa-node','0','--search-dram-budget-gib',str(budget),'--fixed-beam',str(BEAM),
                    ...
                    try:
                        ...
                        # A failed native reference can leave its target directory
                        # behind; remove only this new run's incomplete reference
                        # artifacts before the authorized retry.
                        raw_root = ROOT/'results'/experiment/dataset/run_id/'raw'
                        for ref in raw_root.glob('*/*/*.reference'):
                            try: shutil.rmtree(ref)
                            except OSError: pass
                        repair_glass(dataset,experiment,run_id,folder,env)
                        command(cmd,folder/'run.log')
                    source=load_diagnostic(f'results/{experiment}/{dataset}/{run_id}')
                    assert len(source[1])==(36 if experiment.startswith('03') else 9)
                    sources.append(source);job.update(status='completed',formal_ready=False);save()
