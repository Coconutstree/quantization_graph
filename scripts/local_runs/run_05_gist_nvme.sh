...

{
  ...
  DATASETS=gist OUT_ROOT=results/archive/legacy_layout_20260918/disk_environment PYTHON=python3 bash legacy/scripts/run_master_round2.sh

  SHARED_GRAPH="results/archive/legacy_layout_20260918/disk_environment/dataset_artifacts/gist/indexes/02_diskann_fair/shared_graph/diskann_fp32_R64_Lbuild400_alpha1.2_seed20260813.graph.bin"
  if [[ ! -s "$SHARED_GRAPH" ]]; then
    src/graph_core/target/release/run_diskann_fair \
      ...
      --query-path results/archive/legacy_layout_20260918/disk_environment/03_system_fair/gist/csv/_query_splits/test_query.fvecs \
      --gt-path results/archive/legacy_layout_20260918/disk_environment/03_system_fair/gist/csv/_query_splits/test_gt.ivecs
  fi
