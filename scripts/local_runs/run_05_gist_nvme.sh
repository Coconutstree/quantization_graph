...

{
  ...
  SHARED_GRAPH="results/disk_environment/dataset_artifacts/gist/indexes/02_diskann_fair/shared_graph/diskann_fp32_R64_Lbuild400_alpha1.2_seed20260813.graph.bin"
  if [[ ! -s "$SHARED_GRAPH" ]]; then
    experiments/02_diskann_fair/target/release/run_diskann_fair \
      ...
      --query-path results/disk_environment/03_system_fair/gist/csv/_query_splits/test_query.fvecs \
      --gt-path results/disk_environment/03_system_fair/gist/csv/_query_splits/test_gt.ivecs
  fi
