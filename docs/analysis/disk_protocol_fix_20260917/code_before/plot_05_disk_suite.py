...
def _single_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Validate the single-run formal protocol."""
    return single_test_rows(rows)
...
def _load_public_rows(public_root: Path, layer: str, dataset: str) -> list[dict[str, Any]]:
    path = public_root / PUBLISH_LAYER_DIRS[layer] / dataset / "csv" / "formal_test_rows.csv"
    if not path.exists():
        raise ContractError(f"missing published single-run CSV: {path}")
        ...
        for layer in layers:
            ...
            for dataset in datasets:
                ...
                if not published_layout:
                    ...
                    rows = rows if os.environ.get("QG05_FAST") == "1" else _single_rows(rows)
