...
def single_test_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    """Validate one measured repeat per operating point without synthesizing statistics."""
    ...
    for row in rows:
        if str(row.get("repeat_id")) not in ("0", "0.0"):
            raise ContractError("formal single-run results require repeat_id=0")
        key = tuple(str(row.get(k, "")) for k in keys)
        if key in seen:
            raise ContractError(f"duplicate formal operating point: {key}")
        seen.add(key)
...
METHOD_SPECS: tuple[MethodSpec, ...] = (
    ...
    MethodSpec("05c", "SymphonyQG-DiskPort", "03_system_fair", "official SymphonyQG FastScan LUT+SIMD", ("hybrid_disk",), "algorithm_preserving_disk_port", "supplementary"),
    ...
    MethodSpec("05c", "AiSAQ-Disk", "official_disk_baselines", "official AiSAQ PQFlashIndex with use_aisaq", ("hybrid_disk",), "official_native_disk"),
...
def validate_artifact(
    ...
    *,
    ...
    if phase in ("validation", "test"):
        ...
        if spec.layer in ("05b", "05c") and artifact.get("storage_cache_protocol") not in ("controlled_cold", "controlled_warm"):
            errors.append("storage cache protocol has not been verified")
        if artifact.get("memory_accounting_complete") is False:
            errors.append("search memory accounting is incomplete")
        elif spec.layer in ("05b", "05c") and artifact.get("memory_accounting_complete") is not True:
