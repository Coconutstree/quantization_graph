...
def parser():
    ...
    p.add_argument('--search-memory-gib', type=float, default=4.0)
...
def main(argv=None):
    ...
    if not args.execute:
        ...
        return 0
    folder = METHODS[args.method]
    if not 0 < args.search_memory_gib < 1048576:
        raise ValueError('search memory budget must be finite and positive')
    memory_environment = check_environment(args.cgroup_parent, int(args.search_memory_gib * (1 << 30)))
    ...
    metadata = {'method': args.method, 'source_commit': head, 'formal_ready': False,
                ...
                'note': 'Official CLI diagnostics with cgroup search limit; formal throughput, query and algorithm acceptance still required.'}
    ...
    for i, cmd in enumerate(plan):
        ...
        if i == len(plan) - 1:
            evidence = run_measured(cmd, evidence_path=args.work_dir/'search.resources.json',
                                    log_path=logfile, cwd=REPO,
                                    env=dict(env, OMP_NUM_THREADS=str(args.workers)),
                                    budget_bytes=int(args.search_memory_gib * (1 << 30)),
