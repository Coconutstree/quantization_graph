...
from diskfair.protocol import (PROTOCOL_ID, DEFAULT_BUDGET_GIB, BUDGET_GRID_GIB,
                               aggregate_repeats, attach_resource_evidence)
from diskfair.memory_runner import run_measured, check_environment, MemoryEnvironmentError
...
def _work_items(
    ...
    # Each run_id has exactly one budget/cache/worker condition, also used in tuning.
    ...
    for repeat_id in range(repeats if phase == "test" else 1):
        for wi, worker in enumerate((1,) if phase == "export" else workers):
            ...
            result.extend(WorkItem(spec, mode, repeat_id, worker, budget_gib, primary_cache)
...
def _publish_disk_environment(run_root: Path, out_root: Path, run_id: str, layer: str, dataset: str) -> None:
    ...
    if not source.exists():
        return
    destination = _publish_root(out_root) / PROTOCOL_ID / run_id / PUBLISH_LAYER_DIRS[layer] / dataset
    ...
    *,
    ...
    if measured:
        ...
        evidence = run_measured(
            ...
            budget_bytes=None if reference else int(item.budget_gib * (1 << 30)),
            cgroup_parent=None if reference else cgroup_parent, reference=reference,
            cpu_affinity=cpu_affinity, numa_node=numa_node,
            env=dict(os.environ, OMP_NUM_THREADS=str(item.workers), MKL_NUM_THREADS="1",
...
def _run_layer_dataset(
    *,
    ...
    items = _work_items(specs, native_phase, workers, args.repeats, dataset, storage_modes,
                        args.search_dram_budget_gib, args.cache_mode)
    if not items:
        raise ContractError(
...
def make_parser(default_layer: str | None = None) -> argparse.ArgumentParser:
    ...
    parser.add_argument("--search-dram-budget-gib", type=float, default=DEFAULT_BUDGET_GIB)
    parser.add_argument("--cgroup-parent", type=Path, default=os.environ.get("QG05_CGROUP_PARENT"))
    ...
    parser.add_argument("--experiment-group", choices=("primary", "budget_scan", "thread_scaling"), default="primary")
...
def run(argv: list[str] | None = None, default_layer: str | None = None) -> int:
    ...
    try:
        ...
        if args.experiment_group != "budget_scan" and args.search_dram_budget_gib != DEFAULT_BUDGET_GIB:
            raise ContractError("non-primary budgets require --experiment-group budget_scan")
        ...
        run_root = args.out_root / "runs" / args.run_id
        config = {"protocol_id": PROTOCOL_ID, "workers": list(args.workers),
                  "budget_gib": args.search_dram_budget_gib, "cache_mode": args.cache_mode,
        ...
        config_path = run_root / "manifests" / "protocol.json"
        ...
        if args.phase in ("validate", "tune", "run"):
            readiness_path = run_root / "manifests" / f"memory_environment_{args.phase}.json"
            try:
        ...
        invocation_path = run_root / "manifests" / f"invocation_{args.phase}.json"
