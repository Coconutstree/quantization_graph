...
def _load_complete(run_root: Path, layer: str, dataset: str) -> list[dict[str, Any]]:
    from diskfair.protocol import PROTOCOL_ID, validate_resource_evidence
    ...
    for source in sources:
        ...
        validate_resource_evidence(artifact, dict(artifact, artifact_path=str(path.resolve())))
        artifact_paths.add(str(path))
    rows = _read_rows(csv_path)
    if {r.get("artifact_path") for r in rows} != artifact_paths:
    ...
    for row_index, dataset in enumerate(datasets):
        ...
        ax.text(
            ...
            (f"{DATASET_LABELS.get(dataset, dataset)} | "
             f"{float(dataset_rows[0]['search_dram_budget_gib']):g} GiB cgroup | "
    ...
    try:
        ...
        for layer in layers:
            ...
            for dataset in datasets:
                ...
                conditions = {(r["search_dram_budget_gib"], r["cache_mode"]) for r in rows}
                if len(conditions) != 1:
                    raise ContractError("plot requires one budget/cache condition")
