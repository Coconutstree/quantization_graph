...
GIST、AGNews、DBpedia 的具体公平实验设计、原算法不改的准入边界与高维支持核验见 [磁盘 baseline 公平对比方案](DISK_BASELINE_FAIR_COMPARISON_20260916.md)。正式实验以该方案的准入要求为准。
...
默认打印命令计划，加 `--execute` 执行。work-dir 必须是新目录，避免混合旧图/结果。
...
2. 全进程内存上限及分类记账：索引、查询工作区、所有线程、可选缓存与页缓存。
...
以相同设备、完整 DRAM 上限、查询集、k、并发工作负载和相同 Recall@10 比较。各方法保留官方图、码长、驻留策略与搜索调度；通过独立验证集选择配置。无需强制相同 PQ 字节或相同 L。Starling 导航图、AiSAQ inline/rearrange/cache 等是方法本身可调配置，不能删除后仍声称完整原方法。
