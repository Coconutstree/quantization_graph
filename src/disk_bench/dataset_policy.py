...
def resident_budget_errors(
    ...
    # 03 Ours now chooses a resident PCA prefix when full DB1 does not fit.
    # The native planner admits codes + factors + projection + worker/runtime
    # reserves after export; rejecting on original DB1 alone would bypass it.
    minimum_route_bytes = base_count * (8 + 20)  # native minimum: 64 bits + five FP32 factors
    if "05c" in layers and minimum_route_bytes > budget_bytes:
        errors.append(
            f"{dataset}: even minimum PCA+1bit codes and factors need "
            f"{minimum_route_bytes / (1 << 30):.2f} GiB before workers/cache, "
            f"exceeding the {budget_gib:g} GiB 05C budget"
        )
    return errors
