"""External measurement: planned RAM budget with observed RSS admission.

RSS budget mode adds no OS memory cap and needs no cgroup delegation. Explicit
cgroup mode remains available as a separate experiment. Lifecycle wall time
is NOT query QPS time.
"""
...
CGROUP_PROTOCOL_ID = 'disk_cgroup_explicit_20260921'
...
def run_measured_isolated(command, **kwargs):
    """Use a fresh lightweight supervisor after offline NumPy/index preparation.

    The existing native-child wait4 + RSS/HWM accounting is unchanged. A new
    supervisor prevents a large preparer's address space/high-water from being
    inherited by the measured fork/exec launcher. Supervisor RSS is not a native
    search metric, just as the ordinary orchestrator is not a search metric.
    """
    child_env = kwargs.pop('env', None)
    request = json.dumps(dict(command=[str(x) for x in command], kwargs=kwargs), default=str)
    result = subprocess.run([sys.executable, str(Path(__file__).resolve()), '--measure-worker'],
                            input=request, text=True, capture_output=True, env=child_env)
    if result.returncode:
        raise MemoryEnvironmentError('isolated measurement failed: ' + result.stderr[-4000:])
...
def run_measured(command, *, evidence_path, log_path, budget_bytes=None,
                 ...
                 cpu_affinity=None, numa_node=None, observe_only=False, rss_budget=False):
    ...
    if reference and observe_only:
        raise ValueError('resident reference and native observation are distinct modes')
    if rss_budget and (reference or observe_only or cgroup_parent is not None):
        raise ValueError('RSS budget is distinct from reference, observation and cgroup modes')
    if rss_budget and (type(budget_bytes) is not int or budget_bytes <= 0):
        raise ValueError('RSS budget must be a positive integer in bytes')
    capped = not (reference or observe_only or rss_budget)
    if capped and (budget_bytes is None or cgroup_parent is None):
    ...
    if not capped and not rss_budget and (budget_bytes is not None or cgroup_parent is not None):
        raise ValueError('unconstrained measurement cannot claim a cgroup budget')
    samples_path = evidence_path.with_suffix('.rss.jsonl')
    ...
    evidence = {
        'protocol_id': CGROUP_PROTOCOL_ID if capped else PROTOCOL_ID, 'status': 'running', 'command': command,
        'binary_sha256': digest(executable), 'memory_enforcement': 'cgroup_v2' if capped else 'none',
        'memory_limit_bytes': budget_bytes if capped else None,
        'planned_budget_bytes': budget_bytes if rss_budget else None,
        'rss_budget': rss_budget, 'budget_admitted': False, 'memory_limit_scope': (
            'planned_process_ram_budget_rss_admission' if rss_budget else
            'unconstrained_reference' if reference else 'process_rss_only_no_added_hard_limit' if observe_only
        ...
        'category_attribution_complete': False, 'reference_only': reference, 'observe_only': observe_only,
        'budget_verified': False,
        'launch_policy': policy,
        'protocol_environment': {key: str((env if env is not None else os.environ)[key])
            for key in ('QG03_WARMUP_BIN', 'QG03_WARMUP_COUNT', 'QG05_OFFICIAL_TRACE_PREFIX')
            if key in (env if env is not None else os.environ)},
    }
    group = None
    try:
        if capped:
    ...
    peaks = {'VmPeak': 0, 'VmHWM': 0, 'VmRSS': 0, 'VmSwap': 0}
    evidence['rss_samples_path'] = str(samples_path.resolve())
    phase, width, log_offset = 'loading', None, 0
    try:
        ...
        with log_path.open('x') as log, samples_path.open('x') as samples:
            log.write('argv=' + json.dumps(command) + '\n'); log.flush()
            child_env = dict(os.environ if env is None else env)
            if rss_budget:
                child_env['QG05_MEASURE_WHOLE_PROCESS'] = '1'
            proc = subprocess.Popen(child, cwd=cwd, env=child_env, stdout=log,
            ...
            while True:
                with log_path.open() as reader:
                    ...
                    for line in reader:
                        ...
                        if ' search:' in line: phase = 'warmup'
        ...
        evidence.update(exit_code=proc.returncode, elapsed_seconds=time.monotonic()-started,
                        ...
                        process_read_bytes=int(usage.ru_inblock)*512,
                        process_write_bytes=int(usage.ru_oublock)*512,
                        process_io_scope='Linux wait4 block IO; entire process including load and warmup',
                        sampled_vm_peak_bytes=peaks['VmPeak'], sampled_rss_high_water_bytes=peaks['VmHWM'],
                        sampled_swap_bytes=peaks['VmSwap'], launch=launch)
        evidence['rss_samples_sha256'] = digest(samples_path)
        evidence['wait4_peak_rss_bytes'] = int(usage.ru_maxrss)*1024
        evidence['observed_peak_rss_bytes'] = max(int(usage.ru_maxrss)*1024, peaks['VmHWM'], peaks['VmRSS'])
        evidence['rss_note'] = 'max sampled RSS/HWM and wait4; HWM resets may hide brief peaks; excludes descendants'
        if rss_budget:
            evidence['process_peak_rss_bytes'] = evidence['observed_peak_rss_bytes']
            evidence['process_peak_rss_source'] = 'max_wait4_sampled_rss_hwm_linux_bytes'
        if group:
        ...
        if events.get('oom_kill', 0) or events.get('oom_group_kill', 0):
            ...
            state = 'runtime_error'
        elif capped and (evidence['cgroup_memory_peak_bytes'] > budget_bytes or events.get('oom', 0)):
            state = 'resource_review_required'
        elif rss_budget and evidence['observed_peak_rss_bytes'] > budget_bytes:
            state = 'budget_exceeded'
        elif rss_budget and peaks['VmSwap'] > 0:
            state = 'swap_observed'
        else:
            state = 'completed'
        evidence.update(status=state, budget_verified=(state=='completed' and capped
                            and launch.get('joined_cgroup')==str(group.path)
                            and evidence['memory_limit_bytes']==budget_bytes
                            and evidence['swap_limit_bytes']==0))
        evidence['budget_admitted'] = bool(rss_budget and state == 'completed')
        return evidence
...
def main():
    if len(sys.argv)>1 and sys.argv[1]=='--measure-worker':
        ...
        result = run_measured(request['command'], **request['kwargs'])
    ...
    p.add_argument('--budget-gib', type=float, default=2)
    ...
    try:
        if a.check:
            print(json.dumps(check_environment(a.cgroup_parent,budget) if a.memory_mode == 'cgroup' else
                             {'status':'ready', 'memory_enforcement':'none', 'planned_budget_bytes':budget},indent=2)); return 0
        ...
        result=run_measured(command,evidence_path=a.output/'memory.json',log_path=a.output/'terminal.log',
                            cgroup_parent=a.cgroup_parent,budget_bytes=budget,timeout=a.timeout, rss_budget=a.memory_mode=='rss_budget')
        print(json.dumps(result,indent=2));return 0 if result['budget_verified'] or result['budget_admitted'] else 2
