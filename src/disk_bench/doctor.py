...
def _ours_graph_candidates(repo_root: Path, dataset: str) -> tuple[Path, ...]:
    ...
    return (
        repo_root / "artifacts" / "graphs" / dataset / "Ours" / filename,
        repo_root / "artifacts" / "indexes" / "legacy_aliases" / dataset / "indexes" / "02_diskann_fair" / "Ours" / filename,
    ...
    *,
    ...
    for dataset in datasets:
        ...
        if "05b" in layers:
            ...
            shared_candidates = (
                repo_root / "artifacts" / "graphs" / dataset / "shared_graph" / shared_filename,
                repo_root
    ...
    try:
        ...
        for spec in METHOD_SPECS:
            if spec.layer not in layers or spec.comparison_group != "primary":
                continue
            port = ports[spec.key]
    ...
    print("Disk experiments 01/02/03 doctor")
    for check in checks:
