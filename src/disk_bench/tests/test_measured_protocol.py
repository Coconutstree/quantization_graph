...
from orchestrator import _work_items, specs_for, make_parser, configure_experiment, _dataset_admission_errors
...
def rows():
    return [dict(protocol_id=PROTOCOL_ID, formal_ready=True, repeat_id=0,
                 ...
                 storage_mode='hybrid_disk', cache_mode='standard', search_dram_budget_gib=2,
...
class MatrixTests(unittest.TestCase):
    def test_primary_matches_tuning(self):
        ...
        args=make_parser('03').parse_args(['--phase','run']);configure_experiment(args,'03')
        for phase,count in [('validation',1),('test',1)]:
            items=_work_items(spec,phase,(32,),1,'gist',('hybrid_disk',),args.search_dram_budget_gib)
    ...
    def test_ours_minimum_route_bound_does_not_block_other_methods(self):
        ...
        with patch.object(mod,'_fvec_shape',return_value=(114000000,1024)):
            ...
            # Full DB1 does not fit, but 64-bit PCA codes + factors do: the
            # full planner must decide, rather than rejecting on original D.
            self.assertEqual(_dataset_admission_errors('msmarco',('05c',),Path('/tmp'),4,('Ours-Disk',)),[])
            self.assertTrue(_dataset_admission_errors('msmarco',('05c',),Path('/tmp'),2,('Ours-Disk',)))


class ResourceTests(unittest.TestCase):
    def test_large_reference_does_not_contaminate_fresh_measurement(self):
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            budget = 192 * (1 << 20)
            def run(name, mib, reference=False):
                return run_measured(
                    [sys.executable, '-c', f'a=bytearray({mib}*1024*1024)'],
                    evidence_path=root / f'{name}.json', log_path=root / f'{name}.log',
                    reference=reference, rss_budget=not reference,
                    budget_bytes=None if reference else budget)
            ref = run('reference', 256, True)
            measured = run('measured', 8)
            oversized = run('oversized', 256)
            self.assertEqual(ref['status'], 'completed')
            self.assertGreater(ref['process_peak_rss_bytes'], budget)
            self.assertFalse(ref['budget_admitted'])
            self.assertLessEqual(ref['finished_unix'], measured['started_unix'])
            self.assertNotEqual(ref['pid'], measured['pid'])
            self.assertEqual(measured['status'], 'completed')
            self.assertLessEqual(measured['process_peak_rss_bytes'], budget)
            self.assertTrue(measured['budget_admitted'])
            self.assertEqual(oversized['status'], 'budget_exceeded')
            self.assertFalse(oversized['budget_admitted'])
...
class PlotAcceptanceTests(unittest.TestCase):
    def test_legacy_manifest_rejected_even_with_fast_mode(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            root=Path(d)/'manifests'/'fixture';base=Path(d)/'03_disk_system'/'gist'/'fixture'/'tables';base.mkdir(parents=True)
