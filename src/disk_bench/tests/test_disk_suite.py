...
def test_dataset_policy_budget_lower_bounds():
    ...
    assert msmarco and "minimum PCA+1bit" in msmarco[0]
    # Full 1024-bit codes alone exceed 0.5 GiB, but a PCA prefix can fit;
    # defer the complete decision to the native planner, including workers.
    assert not resident_budget_errors("gist", 5_000_000, 1024, ("05c",), .5)
