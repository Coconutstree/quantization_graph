...
use crate::ours_diskann::{
    ...
    load_adaptive_route, search_ours_paper_active, OursPaperSearchStats, OursPrecursor, RabitqSpace,
};
...
pub fn run_ours_exrabitq4(
    ...
    if let Some(route_dir) = &ctx.config.adaptive_route_dir {
        let route_dim = ctx.config.adaptive_route_dim;
        if route_dim == 0 {
            return Err("--adaptive-route-dim must be positive when --adaptive-route-dir is set".to_string());
        }
        append_progress(progress_log, "adaptive_route_load", &format!(
            "start dir={} dim={} rows={} input_dim={}", route_dir.display(), route_dim, data.nrows(), data.ncols()))?;
        let route = load_adaptive_route(route_dir, data.nrows(), route_dim, data.ncols()).map_err(err)?;
        index.provider().aux_vectors.install_adaptive_route(route).map_err(err)?;
        append_progress(progress_log, "adaptive_route_load", "done")?;
    }
    ...
    let query_limit = std::env::var("QGRAPH_QUERY_LIMIT")
        .ok().and_then(|v| v.parse::<usize>().ok())
        .map(|v| v.min(queries.nrows())).unwrap_or(queries.nrows());
        ...
        for &search_list_size in &ctx.config.search_list_sizes {
            ...
            for repeat_idx in 0..ctx.config.repeats {
                for qid in 0..query_limit {
                    ...
                    total += K as u64;
                    if qid % 10 == 0 || qid + 1 == query_limit {
                        append_progress(
                            progress_log,
                            "search_progress",
                            &format!(
                                "codec={} list={} repeat={} query={}/{} elapsed_s={:.3}",
                                codec.as_str(), search_list_size, repeat_idx,
                                qid + 1, query_limit, search_start.elapsed().as_secs_f64()
                            ),
                        )?;
                    }
                }
