...
fn run() -> Result<(), String> {
    ...
    if args.build_only {
        // An empty sweep preserves graph construction and disables every search loop.
        args.search_list_sizes.clear();
    }
    ...
    println!("build_only={}", args.build_only);
    ...
    for method in &args.methods {
        ...
        let mut prepared = adapter.prepare(&ctx, &paths.raw_log)?;
        if args.build_only {
            if !prepared.search_results.is_empty() {
                return Err("build-only unexpectedly executed searches".to_string());
            }
            prepared.has_real_metrics = false;
            prepared.note.push_str(" build_only=true; no query performance measured");
        }
