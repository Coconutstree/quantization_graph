...
pub struct OursStore {
    ...
    adaptive_route: UnsafeCell<Option<AdaptiveRoute>>,
}
...
impl AdaptiveRoute {
    pub(crate) fn load_norms(&mut self, path: &Path, calibrate: bool) -> ANNResult<()> {
        let bytes = fs::read(path)?;
        if bytes.len() != self.scales.len() * 4 {
            return Err(ANNError::message("route norm row count mismatch"));
        }
        let norms: Vec<f32> = bytes.chunks_exact(4)
            .map(|b| f32::from_le_bytes(b.try_into().unwrap())).collect();
        if norms.iter().any(|v| !v.is_finite() || *v < 0.0) {
            return Err(ANNError::message("invalid route norm"));
        }
        if calibrate {
            for (scale, norm) in self.scales.iter_mut().zip(&norms) {
                if *scale > 0.0 { *scale = *norm / (self.dim as f32 * *scale); }
                else if *norm != 0.0 {
                    return Err(ANNError::message("nonzero norm with zero route scale"));
                }
            }
        }
        self.norms = norms;
    ...
    pub(crate) fn resident_bytes(&self) -> usize {
        self.codes.capacity() + 4 * (self.norms.capacity() + self.scales.capacity() + self.mean.capacity() + self.components.capacity())
    }
    pub(crate) fn project(&self, query: &[f32]) -> Vec<f32> {
        assert_eq!(query.len(), self.mean.len());
        (0..self.dim).map(|r| query.iter().enumerate().map(|(c, q)|
            (q - self.mean[c]) * self.components[r * query.len() + c]).sum()).collect()
    }
    pub(crate) fn score(&self, id: usize, projected: &[f32]) -> f32 {
        assert_eq!(projected.len(), self.dim);
        let s = self.scales[id];
        let row = &self.codes[id * self.bytes_per_row..(id + 1) * self.bytes_per_row];
        let dot: f32 = projected.iter().enumerate().map(|(i, q)|
            if row[i / 8] & (1 << (i % 8)) != 0 { *q } else { -*q }).sum();
        let norm = self.norms.get(id).copied().unwrap_or(self.dim as f32 * s * s);
        norm - 2.0 * s * dot
    }
}

#[cfg(test)]
mod adaptive_numerics_tests {
    use super::*;
    #[test]
    fn projection_npy_validates_shape_length_and_values() {
        let path = std::env::temp_dir().join(format!("projection_shape_{}.npy", std::process::id()));
        let header = "{'descr': '<f4', 'fortran_order': False, 'shape': (2, 3), }\n";
        let mut bytes = b"\x93NUMPY\x01\x00".to_vec();
        bytes.extend_from_slice(&(header.len() as u16).to_le_bytes());
        bytes.extend_from_slice(header.as_bytes());
        for value in 0..6 { bytes.extend_from_slice(&(value as f32).to_le_bytes()); }
        fs::write(&path, &bytes).unwrap();
        assert_eq!(read_f32_npy(&path, &[1, 3], true).unwrap(), vec![0., 1., 2.]);
        assert!(read_f32_npy(&path, &[3, 2], false).is_err());
        assert!(read_f32_npy(&path, &[6], false).is_err());
        assert!(read_f32_npy(&path, &[3, 3], true).is_err());
        let offset = 10 + header.len();
        bytes[offset..offset + 4].copy_from_slice(&f32::NAN.to_le_bytes());
        fs::write(&path, &bytes).unwrap();
        assert!(read_f32_npy(&path, &[2, 3], false).is_err());
        bytes.pop();
        fs::write(&path, &bytes).unwrap();
        assert!(read_f32_npy(&path, &[2, 3], false).is_err());
    ...
    #[test]
    fn norm_calibration_recovers_self_distance() {
        let route = AdaptiveRoute { norms: vec![5.0], codes: vec![3],
            scales: vec![5.0/3.0], mean: vec![0.,0.],
            components: vec![1.,0.,0.,1.], dim: 2, bytes_per_row: 1 };
        assert!((route.score(0, &[1.,2.]) + 5.0).abs() < 1e-6);
        assert_eq!(route.resident_bytes(), 1+4*(1+1+2+4));
    }
    #[test]
    fn asymmetric_score_matches_reconstruction() {
        let route = AdaptiveRoute { norms: Vec::new(), codes: vec![0b10101], scales: vec![0.25],
            mean: vec![1., 2.], components: vec![1.,0., 0.,1., 1.,1., -1.,0., 0.,-1.],
            dim: 5, bytes_per_row: 1 };
        let q = route.project(&[3., 5.]);
        let explicit: f32 = q.iter().enumerate().map(|(i, x)| {
            let b = if i % 2 == 0 { 0.25 } else { -0.25 };
            (x-b)*(x-b)
        }).sum();
        let norm: f32 = q.iter().map(|x| x*x).sum();
        assert!((route.score(0, &q) + norm - explicit).abs() < 1e-5);
    }
    #[test]
    fn fp16_subnormals_are_not_half() {
        assert_eq!(half_to_f32(1), 2.0_f32.powi(-24));
        assert_eq!(half_to_f32(0x8001), -2.0_f32.powi(-24));
...
fn half_to_f32(bits: u16) -> f32 {
    if bits & 0x7c00 == 0 {
        let magnitude = (bits & 0x03ff) as f32 * 2.0_f32.powi(-24);
    ...
    let value = if exp == 0 { sign | if frac == 0 { 0 } else { 0x3f000000 } }
        else if exp == 31 { sign | 0x7f800000 | (frac << 13) }
        else { sign | ((exp + 112) << 23) | (frac << 13) };
    f32::from_bits(value)
}

fn read_f32_npy(path: &Path, expected: &[usize], row_prefix: bool) -> ANNResult<Vec<f32>> {
    let invalid = || ANNError::message("invalid projection npy shape or size");
    let count = expected.iter().try_fold(1usize, |a, b| a.checked_mul(*b)).ok_or_else(invalid)?;
    let bytes = fs::read(path).map_err(|e| ANNError::message(format!("{}: {e}", path.display())))?;
    if bytes.len() < 12 || &bytes[..6] != b"\x93NUMPY" {
        return Err(ANNError::message(format!("invalid npy: {}", path.display())));
    }
    let major = bytes[6];
    ...
    } else { return Err(ANNError::message("unsupported npy version")); };
    let payload = (start as usize).checked_add(header_len).ok_or_else(invalid)?;
    if payload > bytes.len() { return Err(invalid()); }
    let header = std::str::from_utf8(&bytes[start..payload])
        .map_err(|_| ANNError::message("invalid npy header encoding"))?;
    if !header.contains("'descr': '<f4'") || !header.contains("'fortran_order': False") {
        return Err(ANNError::message("projection requires C-order little-endian float32 npy"));
    }
    let shape_text = header.split("'shape'").nth(1).ok_or_else(invalid)?;
    let shape_text = shape_text.trim_start().strip_prefix(':').ok_or_else(invalid)?
        .trim_start().strip_prefix('(').ok_or_else(invalid)?;
    let shape_text = shape_text.split_once(')').ok_or_else(invalid)?.0;
    let shape = shape_text.split(',').map(str::trim).filter(|s| !s.is_empty())
        .map(|s| s.parse::<usize>().map_err(|_| invalid())).collect::<ANNResult<Vec<_>>>()?;
    if shape.len() != expected.len() || shape.iter().zip(expected).enumerate()
        .any(|(i, (a, b))| if row_prefix && i == 0 { a < b } else { a != b }) {
        return Err(invalid());
    }
    let stored_count = shape.iter().try_fold(1usize, |a, b| a.checked_mul(*b)).ok_or_else(invalid)?;
    if stored_count.checked_mul(4).and_then(|n| payload.checked_add(n)) != Some(bytes.len()) {
        return Err(invalid());
    }
    let end = count.checked_mul(4).and_then(|n| payload.checked_add(n)).ok_or_else(invalid)?;
    let values: Vec<f32> = bytes[payload..end].chunks_exact(4)
        .map(|x| f32::from_le_bytes([x[0], x[1], x[2], x[3]])).collect();
    if values.iter().any(|v| !v.is_finite()) { return Err(ANNError::message("non-finite projection")); }
    Ok(values)
}

pub(crate) fn load_adaptive_route(dir: &Path, rows: usize, route_dim: usize, input_dim: usize) -> ANNResult<AdaptiveRoute> {
    if rows == 0 || route_dim == 0 || route_dim > input_dim {
        return Err(ANNError::message("invalid adaptive dimensions"));
    }
    let bytes_per_row = route_dim.div_ceil(8);
    let codes = fs::read(dir.join(format!("codes_d{route_dim}.bin")))
        .map_err(|e| ANNError::message(format!("adaptive codes: {e}")))?;
    if codes.len() != rows * bytes_per_row { return Err(ANNError::message("adaptive code row count mismatch")); }
    let scale_bytes = fs::read(dir.join(format!("scales_d{route_dim}.f16")))
        .map_err(|e| ANNError::message(format!("adaptive scales: {e}")))?;
    if scale_bytes.len() != rows * 2 { return Err(ANNError::message("adaptive scale row count mismatch")); }
    let scales: Vec<f32> = scale_bytes.chunks_exact(2).map(|x| half_to_f32(u16::from_le_bytes([x[0], x[1]]))).collect();
    if scales.iter().any(|x| !x.is_finite() || *x < 0.0) {
        return Err(ANNError::message("invalid adaptive scale"));
    }
    let projection_dir = dir.parent().unwrap_or(dir).join("projection_nnp");
    let mean = read_f32_npy(&projection_dir.join("mean.f32.npy"), &[input_dim], false)?;
    let components = read_f32_npy(&projection_dir.join("components.f32.npy"), &[route_dim, input_dim], true)?;
    Ok(AdaptiveRoute { norms: Vec::new(), codes, scales, mean, components, dim: route_dim, bytes_per_row })
}
...
impl OursStore {
    fn new(space: Arc<RabitqSpace>, total: usize, prefetch_lookahead: usize) -> Self {
        ...
        Self {
            ...
            write_locks,
            prefetch_lookahead,
            adaptive_route: UnsafeCell::new(None),
        }
    }

    pub(crate) fn install_adaptive_route(&self, route: AdaptiveRoute) -> ANNResult<()> {
        let mut route = route;
        // DiskANN reserves one frozen/start slot in addition to the base rows.
        // The offline route artifact is keyed only by base rows, so append a
        // harmless zero-scale sentinel for that reserved slot.
        if route.scales.len() + 1 == self.total && route.codes.len() == (self.total - 1) * route.bytes_per_row {
            route.scales.push(0.0);
            route.codes.resize(self.total * route.bytes_per_row, 0);
        }
        if route.scales.len() != self.total || route.codes.len() != self.total * route.bytes_per_row
            || route.mean.is_empty() || route.components.len() != route.dim * route.mean.len() {
            return Err(ANNError::message(format!(
                "adaptive route artifact shape mismatch: rows={} scales={} codes={} expected_codes={} dim={} mean={} components={}",
                self.total, route.scales.len(), route.codes.len(), self.total * route.bytes_per_row,
                route.dim, route.mean.len(), route.components.len())));
        }
        unsafe { *self.adaptive_route.get() = Some(route); }
        Ok(())
    }

    fn project_adaptive_query(&self, query: &[f32]) -> Option<Vec<f32>> {
        let route = unsafe { (&*self.adaptive_route.get()).as_ref()? };
        assert_eq!(query.len(), route.mean.len());
        Some((0..route.dim).map(|r| {
            query.iter().enumerate().map(|(c, q)|
                (q - route.mean[c]) * route.components[r * query.len() + c]).sum()
        }).collect())
    }

    pub(crate) fn adaptive_route_score(&self, id: usize, query: &[f32]) -> Option<f32> {
        let route = unsafe { (&*self.adaptive_route.get()).as_ref()? };
        if query.len() != route.dim || id >= self.total { return None; }
        let mut score = 0.0_f32;
        for r in 0..route.dim {
            let projected = query[r];
            let bit = (route.codes[id * route.bytes_per_row + (r >> 3)] & (1_u8 << (r & 7))) != 0;
            score += projected * if bit { route.scales[id] } else { -route.scales[id] };
        }
        Some(route.dim as f32 * route.scales[id] * route.scales[id] - 2.0 * score)
    }
    ...
    Ctx: ExecutionContext,
{
    ...
    let adaptive_query = provider.aux_vectors.project_adaptive_query(query);
    ...
    loop {
        ...
        for node in frontier.iter().copied() {
            ...
            // Adaptive projected 1-bit routing is an optional candidate-order
            // stage.  It never replaces the exact/rerank distance: it only
            // orders the fresh graph neighbors before the existing paper
            // bound and residual rerank logic.
            if let Some(projected) = adaptive_query.as_ref() {
                fresh.sort_by_cached_key(|id| {
                    let score = provider.aux_vectors.adaptive_route_score(
                        id.into_usize(), projected).expect("validated route shape");
                    // IEEE float ordering transformed to an unsigned sortable key.
