...
#   OUT_ROOT=results/archive/legacy_layout_20260918/disk_environment PYTHON=python3 bash legacy/scripts/run_master_round2.sh
#   DATASETS=agnews bash legacy/scripts/run_master_round2.sh
#   DATASETS="agnews gist" M=32 L=200 bash legacy/scripts/run_master_round2.sh
set -uo pipefail
...
PY="${PYTHON:-python3}"
BIN01="${BIN01:-${ROOT}/build/disk/01_disk_quantizer/faiss_quantizer_smoke}"
BIN01_CAND="${BIN01_CAND:-${ROOT}/build/disk/01_disk_quantizer/faiss_hard_negative_candidates}"
BIN02="${BIN02:-${ROOT}/src/graph_core/target/release/run_diskann_fair}"
OUT="${OUT_ROOT:-results/archive/legacy_layout_20260918/disk_environment}"
LOG_DIR="${ROOT}/logs"
...
run_01() {
  ...
  source legacy/01_quantizer_fair/configs/formal_faiss_4bit.env
  ...
  DATASETS="${ds}" RERANK_CANDIDATES="${RERANK}" OUT_ROOT="${OUT}" WORK_ROOT="work" \
    ...
    bash legacy/scripts/run_saq_fixed_candidates_fair.sh \
...
EOF
  ...
  "${PY}" legacy/scripts/export_paper_quantizer_table.py \
...
prepare_query_splits(ds, Path(data_root), Path("results/archive/legacy_layout_20260918/disk_environment"), valq)
...
EOF
  "${PY}" -u legacy/03_system_fair/run_system_fair.py \
    ...
    || log "03 ${ds} FAILED"
  "${PY}" legacy/scripts/plot_system_fair.py --dataset "${ds}" --out-root "${OUT}" \
    >> "${LOG_DIR}/round2_03_${ds}.log" 2>&1 \
