...
CONFIG="${CONFIG:-legacy/01_quantizer_fair/configs/formal_saq_4bit.env}"
if [[ -f "${CONFIG}" ]]; then
...
OUT_ROOT="${OUT_ROOT:-results/archive/legacy_layout_20260918/disk_environment}"
...
: "${DATASETS:?missing DATASETS; example: DATASETS=\"dbpedia\" legacy/scripts/run_saq_fixed_candidates_fair.sh}"
