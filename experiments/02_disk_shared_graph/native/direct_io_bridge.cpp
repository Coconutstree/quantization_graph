...
void* qgraph05_query_cache_new() noexcept {
    try { return new qgraph05::QueryPageCache(); }
    catch (...) { remember_exception(); return nullptr; }
}

void qgraph05_query_cache_delete(void* cache) noexcept {
    delete static_cast<qgraph05::QueryPageCache*>(cache);
}

bool qgraph05_query_cache_get(void* cache, std::uint32_t file,
                            std::uint64_t page, char* output) noexcept {
    return static_cast<qgraph05::QueryPageCache*>(cache)->get(file, page, output);
}

void qgraph05_query_cache_put(void* cache, std::uint32_t file,
                            std::uint64_t page, const char* input) noexcept {
    static_cast<qgraph05::QueryPageCache*>(cache)->put(file, page, input);
}

std::size_t qgraph05_query_cache_bytes(void* cache) noexcept {
    return static_cast<qgraph05::QueryPageCache*>(cache)->allocated_bytes();
}

std::uint64_t qgraph05_query_cache_evictions(void* cache) noexcept {
    return static_cast<qgraph05::QueryPageCache*>(cache)->evictions;
}
