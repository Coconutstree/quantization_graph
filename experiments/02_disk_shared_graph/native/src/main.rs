...
use diskann_fair::disk_port::routing::RoutingPlan;
use diskann_fair::disk_port::pca;
use diskann_fair::disk_port::record_cache::{self, RecordCache, RecordProfile};
const OURS_SYSTEM_KERNEL: &str = "ExRaBitQ4 symmetric Vamana; resident DB1 or budget-selected PCA+1bit M32 (no residual)";

fn formal_route_plan(args: &Args, index: &Path) -> Result<Option<serde_json::Value>> {
    if args.optional("--ours-route-policy").is_none() { return Ok(None); }
    if args.optional("--ours-route-policy") != Some(pca::POLICY) {
        return Err("unknown formal Ours route policy".into());
    }
    if args.optional("--ours-route-plan").is_none()
        && (args.text("--layer")? != "05c" || args.text("--method")? != "Ours-Disk") {
        return Err("automatic PCA policy is only for formal 03 Ours-Disk".into());
    }
    let gib: f64=args.number("--search-dram-budget-gib")?;
    if !gib.is_finite() || gib<=0. || gib*(1u64<<30)as f64>=usize::MAX as f64 {
        return Err("invalid RAM budget".into());
    }
    Ok(Some(pca::plan_dimension(index,args.number("--workers")?,
        (gib*(1u64<<30)as f64).floor() as usize,
        args.number("--ours-query-reservation-bytes")?,
        args.optional("--ours-fixed-route-dimension").map(|v|v.parse::<usize>().map_err(err)).transpose()?)?))
}

type Result<T> = std::result::Result<T, String>;

/// Header-only admission before allocating the resident DB1 arrays.
fn preflight_ours_routing(args: &Args, files: &IndexFiles, meta: &IndexMeta) -> Result<RoutingPlan> {
    let mut header = [0u8; 24];
    let mut file = File::open(&files.ours_sidecar).map_err(err)?;
    file.read_exact(&mut header).map_err(err)?;
    if &header[..8] != b"QG05OSC1" {
        return Err("invalid Ours sidecar magic during memory preflight".into());
    }
    let codes = u64::from_le_bytes(header[8..16].try_into().unwrap());
    let factors = u64::from_le_bytes(header[16..24].try_into().unwrap());
    let routing = codes.checked_add(factors).ok_or("routing size overflow")?;
    if routing.checked_add(24) != Some(file.metadata().map_err(err)?.len()) {
        return Err("Ours sidecar file length mismatch during memory preflight".into());
    }
    let workers: u64 = args.number("--workers")?;
    if workers == 0 { return Err("workers must be positive".into()); }
    let scratch = (meta.base_count as u64).checked_add(12 * 1024 * 1024)
        .and_then(|n| n.checked_mul(workers)).ok_or("worker reservation overflow")?;
    let extra: u64 = args.optional("--routing-other-reservation-bytes").unwrap_or("0")
        .parse().map_err(err)?;
    let safety: u64 = args.optional("--routing-safety-reservation-bytes").unwrap_or("0")
        .parse().map_err(err)?;
    let mut metadata_header = [0u8; 68];
    let mut metadata_file = File::open(&files.ours_metadata).map_err(err)?;
    metadata_file.read_exact(&mut metadata_header).map_err(err)?;
    let field = |offset| u64::from_le_bytes(metadata_header[offset..offset + 8].try_into().unwrap());
    let centroid_bytes = field(12).checked_mul(field(20)).and_then(|n| n.checked_mul(4))
        .ok_or("centroid size overflow")?;
    if &metadata_header[..8] != b"QG05OUR1" || field(12) != meta.dimension as u64
        || field(28) != meta.base_count as u64 || field(20) == 0 || field(28) == 0 || field(12) == 0 || field(52) == 0 || field(60) == 0
        || field(28).checked_mul(field(52)) != Some(codes)
        || field(28).checked_mul(field(60)) != Some(factors)
        || centroid_bytes.checked_add(68) != Some(metadata_file.metadata().map_err(err)?.len()) {
        return Err("Ours routing metadata mismatch during memory preflight".into());
    }
    let mapping=if args.optional("--locality-layout-dir").is_some(){
        (meta.base_count as u64).checked_mul(9).ok_or("mapping reservation overflow")?
    }else{0}; // peak: raw bytes + decoded slots + permutation check
    let record_stride=field(52).checked_add(field(60)).ok_or("routing stride overflow")?;
    let paging_scratch=record_stride.checked_mul(2).and_then(|n|n.checked_add(1024)).and_then(|n|n.checked_mul(64))
        .and_then(|n|n.checked_mul(workers)).ok_or("routing scratch overflow")?;
    let required = routing.checked_add(centroid_bytes).and_then(|n| n.checked_add(scratch)).and_then(|n|n.checked_add(mapping))
        .and_then(|n| n.checked_add(extra)).and_then(|n| n.checked_add(safety))
        .ok_or("routing admission overflow")?;
    let gib: f64 = args.number("--search-dram-budget-gib")?;
    let bytes = gib * (1u64 << 30) as f64;
    if !bytes.is_finite() || bytes < 1.0 || bytes >= u64::MAX as f64 {
        return Err("invalid search DRAM budget".into());
    }
    let declared_budget = bytes.floor() as u64;
    // The process address-space soft limit can be stricter than the CLI budget.
    let limits = fs::read_to_string("/proc/self/limits").map_err(err)?;
    let soft_limit = limits.lines().find_map(|line| line.strip_prefix("Max address space"))
        .and_then(|line| line.split_whitespace().next()).ok_or("missing address-space limit")?;
    let address_limit = if soft_limit == "unlimited" { u64::MAX }
        else { soft_limit.parse::<u64>().map_err(err)? };
    let budget = declared_budget.min(address_limit);
    let state = if required > budget { "does_not_fit" }
        else if required == budget { "fits_exactly" } else { "fits_with_spare" };
    eprintln!("routing_preflight state={state} codes_bytes={codes} factors_bytes={factors} worker_reservation_bytes={scratch} other_reservation_bytes={extra} safety_reservation_bytes={safety} required_bytes={required} budget_bytes={budget} spare_bytes={} estimate_only=true", budget.saturating_sub(required));
    let nonrouting=required-routing;
    let available=budget.checked_sub(nonrouting).ok_or_else(||format!("fixed Ours memory requires {nonrouting} bytes, budget is {budget}"))?;
    let cache=args.optional("--routing-cache-bytes").map(str::parse::<usize>).transpose().map_err(err)?;
    let inflight=args.optional("--routing-max-inflight-pages").unwrap_or("64").parse::<usize>().map_err(err)?;
    let paging_available=if routing>available {available.checked_sub(paging_scratch).ok_or("insufficient memory for paging scratch and layout")?}else{available};
    let mut plan=RoutingPlan::choose(usize::try_from(paging_available).map_err(err)?,usize::try_from(routing).map_err(err)?,cache,inflight)?;
    let factor_mode=args.optional("--routing-factors").unwrap_or("paged");
    if !matches!(factor_mode,"paged"|"resident") {return Err("routing factors must be paged or resident".into());}
    if matches!(plan,RoutingPlan::Paged(_)) {
        let hot_bytes=if let Some(path)=args.optional("--routing-hot-pages") {
            let n=fs::metadata(path).map_err(err)?.len();
            if n%8!=0 || n/8>codes.checked_add(24).ok_or("codes overflow")?.div_ceil(4096) {return Err("invalid hot pages file size".into());}
            n.checked_mul(2).ok_or("hot pages reservation overflow")?
        }else{0};
        let profile_bytes=if args.optional("--routing-profile-output").is_some() {
            codes.checked_add(24).ok_or("codes overflow")?.div_ceil(4096).checked_mul(16).ok_or("profile reservation overflow")?
        }else{0};
        let auxiliary=hot_bytes.checked_add(profile_bytes).ok_or("routing auxiliary overflow")?;
        let resident_factors=if factor_mode=="resident"{factors}else{0};
        let remaining=paging_available.checked_sub(resident_factors).and_then(|n|n.checked_sub(auxiliary))
            .ok_or("insufficient memory for resident factors and routing auxiliary buffers")?;
        plan=RoutingPlan::choose(usize::try_from(remaining).map_err(err)?,usize::try_from(routing-resident_factors).map_err(err)?,cache,inflight)?;
        if let RoutingPlan::Paged(ref mut p)=plan {
            p.resident_factors=factor_mode=="resident";
            p.reserved_bytes=p.reserved_bytes.checked_add(usize::try_from(auxiliary).map_err(err)?).ok_or("routing reservation overflow")?;
            eprintln!("routing_auxiliary hot_reservation_bytes={hot_bytes} profile_reservation_bytes={profile_bytes} resident_factors_bytes={resident_factors}");
        }else{return Err("unexpected resident codes plan after paged admission".into());}
    }
    let policy=args.optional("--routing-cache-policy").unwrap_or("clock");
    if !matches!(policy,"clock"|"lru"){return Err("routing cache policy must be clock or lru".into());}
    if let RoutingPlan::Paged(ref mut p)=plan {p.clock=policy=="clock";}
    if let RoutingPlan::Paged(_) = plan {
        if args.text("--layer")? != "05c" || args.text("--cache-mode")? != "c0"
            || args.optional("--memory-policy").unwrap_or("baseline") != "baseline"
            || args.optional("--memory-cache-bytes").unwrap_or("0") != "0" {
            return Err("paged routing requires 05c C0 and baseline memory policy with zero optional cache".into());
        }
    }
    eprintln!("routing_storage_plan={plan:?} mapping_peak_reservation_bytes={mapping} paging_scratch_reservation_bytes={paging_scratch}");
    Ok(plan)
}
...
fn search_one<C: ResidentCodec>(
    ...
    Ok(QueryRun {
        ...
        latency_us,
        stats,
        graph_io: IoStats::default(), compact_io: IoStats::default(), residual_io: IoStats::default(),
        ablation: "",
...
fn search_one_ours(
    ...
    let latency_us = started.elapsed().as_secs_f64() * 1_000_000.0;
    let OursDiskStats {
        io, graph_io, compact_io, residual_io,
        queue_compute_us,
        rerank_us,
        db1_checks,
        db1_survivors,
    ...
    Ok(QueryRun {
        ...
        ids: result.ids,
        latency_us,
        stats: io, graph_io, compact_io, residual_io,
...
fn execute_config_ours(
    warmup_input: Option<Arc<Matrix<f32>>>,
    ...
    direct: bool,
    shared_cache: Option<SharedPageCache>,
    record_cache: Option<Arc<RecordCache>>,
    record_profile: Option<Arc<RecordProfile>>,
) -> Result<BatchRun> {
    if workers == 0 {
        return Err("workers must be positive".into());
    }
    if let Some(r)=codec.routing.as_ref(){r.clear().map_err(err)?;}
    if let Some(c)=&record_cache {c.clear();}
    ...
    std::thread::scope(|scope| -> Result<()> {
        ...
        for worker_id in 0..workers {
            let codec = codec.clone();
            let queries = queries.clone();
            let warmup_input = warmup_input.clone();
            let order = order.clone();
            ...
            let record_profile = record_profile.clone();
            let next = &next;
            let results = &results;
            let finished_at = &finished_at;
            let ready = ready_tx.clone();
            ...
            handles.push(scope.spawn(move || -> Result<()> {
                ...
                if direct {
                    payload_reader.record_cache = record_cache;
                    payload_reader.record_profile = record_profile;
                ...
                let warm_count = warmup_input.as_ref().map_or(order.len(), |q| q.nrows());
                for warm_index in (worker_id..warmup_queries.min(warm_count)).step_by(workers) {
                    let query_id = if warmup_input.is_some() { warm_index } else { order[warm_index] };
                    let _ = search_one_ours(
                        ...
                        query_id,
                        false,
                        warmup_input.as_ref().unwrap_or(&queries).row(query_id),
                        width,
                        beam,
                        ablation,
                    )?;
        ...
        if let Some(r)=codec.routing.as_ref(){r.log_stats("measurement_start",width);}
        if let Some(c)=&record_cache {c.reset();}
        if let Some(p)=&record_profile {p.reset();}
        measured_start = Some(Instant::now());
        for start in starts {
    ...
    if let Some(r)=codec.routing.as_ref(){r.log_stats("measurement_end",width);}
...
fn formal_widths(args: &Args) -> Result<Vec<usize>> {
    let widths = grid_widths(args)?;
    if let Some(value) = args.optional("--measurement-width") {
        let width: usize = value.parse().map_err(err)?;
        if !widths.contains(&width) { return Err("measurement width outside frozen grid".into()); }
        return Ok(vec![width]);
    }
    Ok(widths)
}

fn grid_widths(args: &Args) -> Result<Vec<usize>> {
    ...
    if let Some(value) = args.optional("--search-widths") {
        if args.text("--layer")? != "05c" || args.text("--method")? != "Ours-Disk" {
            return Err("explicit search widths currently require formal Ours".into());
        }
        let widths = parse_positive_list(value, "--search-widths")?;
        if widths.iter().any(|&w|w<10) || widths.windows(2).any(|w| w[0]>=w[1]) {
            return Err("search widths must be increasing, unique and >= top-k 10".into());
        }
        if args.text("--phase")? == "test" {
            let lock: serde_json::Value = serde_json::from_slice(&fs::read(args.path("--tuning-lock")?).map_err(err)?).map_err(err)?;
            if lock["selected"]["Ours-Disk::hybrid_disk"]["search_widths"] != serde_json::json!(widths) {
                return Err("search widths differ from frozen parameters".into());
            }
        }
        return Ok(widths);
    }
    ...
    if args.text("--layer")? == "05c" {
        return Ok(vec![10, 20, 40, 60, 100, 160, 240, 400, 580]);
    }
...
fn validation_beams(args: &Args) -> Result<Vec<usize>> {
    if let Some(value) = args.optional("--fixed-beam") {
        let beam = value.parse::<usize>().map_err(|_| "invalid --fixed-beam")?;
        if beam == 0 { return Err("--fixed-beam must be positive".into()); }
        return Ok(vec![beam]);
    }
    if std::env::var("QG05_FAST").ok().as_deref() == Some("1") {
        return Ok(vec![1]);
...
fn selected_test_beam(args: &Args) -> Result<usize> {
    ...
    if let Some(value) = args.optional("--fixed-beam") {
        if config != format!("beam{value}") { return Err("fixed beam differs from tuning lock".into()); }
    }
    config
        .strip_prefix("beam")
...
fn write_trace_rows(
    ...
    for run in &batch.runs {
        ...
        let mut row: serde_json::Value = serde_json::from_str(&line).map_err(err)?;
        for (name, io) in [("graph", run.graph_io), ("full4_record", run.compact_io), ("rerank_record", run.residual_io)] {
            row[format!("{name}_io_requests")] = serde_json::json!(io.requests);
            row[format!("{name}_bytes_read")] = serde_json::json!(io.bytes_read);
            row[format!("{name}_shared_cache_hits")] = serde_json::json!(io.shared_cache_hits);
            row[format!("{name}_query_cache_hits")] = serde_json::json!(io.query_cache_hits);
        }
        writeln!(output, "{}", row).map_err(err)?;
    }
    Ok(())
}
...
fn write_search_artifact(
    ...
    if let Some(dir)=args.optional("--pca-route-dir") {
        for name in ["pca.json","assets.json","mean.bin","basis.bin","sidecar.bin"] {
    ...
    let mut fields = vec![
        ...
        ("diagnostic_neighbor_trace", (args.optional("--adaptive-route-trace-dir").is_some() || args.optional("--ours-search-path-dir").is_some()).to_string()),
        ("throughput_comparable", (args.optional("--adaptive-route-trace-dir").is_none() && args.optional("--ours-search-path-dir").is_none()).to_string()),
        ...
        } else if meta.kind==CodecKind::Ours && args.text("--layer")?=="05c" {OURS_SYSTEM_KERNEL}
        ...
        } else if meta.kind==CodecKind::Ours && args.text("--layer")?=="05c" {"native_disk_system"}
        else { PORT_KIND })),
        (
        ...
        ("ours_route_plan", formal_route_plan(args,&files.root)?.unwrap_or(serde_json::Value::Null).to_string()),
        ("pca_assets_sha256", json_escape(args.optional("--pca-assets-sha256").unwrap_or(""))),
        ("pca_route_dir", json_escape(args.optional("--pca-route-dir").unwrap_or(""))),
...
fn run_search_with_codec_factory<C: ResidentCodec + 'static>(
    ...
    let mut cache_pages = if cache_mode == "c0" {
        ...
        // Budget-filling policy: cache_mode=standard means "use the declared RAM budget",
        // not "10% of the base points".  The old `base_count / 10` cap left most of the
        // 4 GiB budget idle while graph pages were still fetched from disk.
        let max_pages_by_nodes = meta.base_count.div_ceil(meta.layout.nodes_per_page);
    ...
    while cached_base_nodes(&cache_pages, &meta.layout, meta.base_count) > meta.base_count {
        cache_pages.pop();
    }
...
fn run_search_with_ours(
    ...
    let independent_warmup = if let Some(path) = args.optional("--warmup-query") {
        if sha256(Path::new(path))? != args.text("--warmup-query-sha256")? {
            return Err("independent warmup hash mismatch".into());
        }
        let q = read_fvecs_matrix(Path::new(path))?;
        if q.ncols() != meta.dimension || q.nrows() < args.number::<usize>("--warmup-queries")? {
            return Err("independent warmup shape/count mismatch".into());
        }
        Some(Arc::new(q))
    } else { None };
    let workers: usize = args.number("--workers")?;
    ...
    let fixed_bytes = if let Some(plan)=formal_route_plan(args,&files.root)? {
        let required=fixed_bytes.checked_add(pca::field(&plan,"other_reserved_bytes")?)
            .and_then(|n|n.checked_add(pca::field(&plan,"pca_query_reservation_bytes").ok()?))
            .ok_or("formal Ours accounting overflow")?;
        if required!=pca::field(&plan,"required_bytes")? {return Err("loaded codec memory differs from routing plan".into());}
        required
    } else {fixed_bytes};
    if fixed_bytes > budget_bytes {
        return Err(format!(
    ...
    let cache_mode = args.text("--cache-mode")?;
    let cache_allocation = args.optional("--ours-cache-allocation").unwrap_or("graph_first");
    if !["graph_first", "records_only"].contains(&cache_allocation) {
        return Err("unknown Ours cache allocation".into());
    }
    let mut cache_pages = if cache_mode == "c0" || cache_allocation == "records_only" {
        Vec::new()
    } else if cache_mode == "standard" {
        let max_pages_by_budget = (budget_bytes - fixed_bytes) / PAGE_SIZE;
        // Budget-filling policy (see the other cache site): no arbitrary fraction cap.
        let max_pages_by_nodes = meta.base_count.div_ceil(meta.layout.nodes_per_page);
        select_bfs_cache_pages(
            &files.graph_pages,
            &meta.layout,
    ...
    let route_plan = formal_route_plan(args, &files.root)?;
    let default_policy = if route_plan.is_some() && cache_mode == "standard" {"hot_dynamic"} else {"off"};
    let record_policy = args.optional("--ours-record-cache-policy").unwrap_or(default_policy);
    if !["off","profile","hot_dynamic"].contains(&record_policy)
        || (record_policy != "off" && (route_plan.is_none() || codec.locality.is_some() || cache_mode != "standard")) {
        return Err("record cache requires formal 03/05 standard packed Ours route".into());
    }
    let profile = if record_policy == "profile" {
        if !["validate","validation"].contains(&args.text("--phase")?) {
            return Err("hot records may only be profiled on validation".into());
        }
        args.path("--ours-record-profile-output")?;
        Some(Arc::new(RecordProfile::new(meta.base_count)))
    } else {None};
    let record_budget = budget_bytes - fixed_bytes - cache_bytes;
    let records = if record_policy == "hot_dynamic" {
        let manifest = args.path("--ours-hot-profile-manifest")?;
        let ranks = args.path("--ours-hot-ranks")?;
        if sha256(&manifest)? != args.text("--ours-hot-profile-sha256")?
            || sha256(&ranks)? != args.text("--ours-hot-ranks-sha256")? {
            return Err("hot-record profile hash mismatch".into());
        }
        let m:serde_json::Value=serde_json::from_slice(&fs::read(&manifest).map_err(err)?).map_err(err)?;
        if m["policy"]!=record_cache::POLICY || m["source_phase"]!="validation"
            || m["route_plan"]!=*route_plan.as_ref().unwrap()
            || m["source_graph_sha256"]!=meta.source_graph_sha256
            || m["native_binary_sha256"]!=args.text("--native-binary-sha256")?
            || m["ranks_sha256"]!=args.text("--ours-hot-ranks-sha256")? {
            return Err("hot-record profile belongs to another route/index/binary".into());
        }
        if args.text("--phase")?!="test" && m["query_sha256"]!=sha256(&args.path("--query")?)? {
            return Err("hot-record profile must come from this validation split".into());
        }
        let mut reader=OursPayloadFactory::direct(&files.ours_payload,codec.layout.clone())
            .map_err(err)?.create().map_err(err)?;
        Some(Arc::new(RecordCache::load(meta.base_count,codec.layout.compact_bytes,
            codec.layout.residual_bytes,record_budget,&ranks,|id|reader.preload_record(id).map_err(err))?))
    } else {None};
    let graph_cache_bytes = cache_bytes;
    let cache_bytes = graph_cache_bytes + records.as_ref().map_or(0,|c|c.allocated);
    let mut record_stats = Vec::new();
        ...
        for &beam in &beams {
            for &width in &widths {
                ...
                let direct = execute_config_ours(
                    ...
                    true,
                    Some(shared_cache.clone()),
                    records.as_ref().filter(|c|c.allocated>0).cloned(),
                    profile.clone(),
                )?;
                if let Some(p)=&profile {p.finish_config();}
                if let Some(c)=&records {
                    record_stats.push(serde_json::json!({"width":width,"beam":beam,"cache":c.stats()}));
                }
                measured_peak_rss_bytes = measured_peak_rss_bytes.max(direct.peak_rss_bytes);
                if let (Some(r),Some(path))=(codec.routing.as_ref(),args.optional("--routing-profile-output")) {
                    r.save_profile(Path::new(path)).map_err(err)?;
                }
                if std::env::var("QG05_FAST").ok().as_deref() != Some("1")
                    && args.optional("--parity-mode") != Some("external") && profile.is_none() {
    ...
    trace.flush().map_err(err)?;
    if let Some(p)=&profile {p.save(&args.path("--ours-record-profile-output")?)?;}
    if parity.count > 0
        && (parity.max_recall_delta > 1e-3
            || parity.mean_overlap() < 0.99
            || parity.mean_visited_delta() > 0.01
    ...
    let mut artifact:serde_json::Value=serde_json::from_slice(&fs::read(&result_path).map_err(err)?).map_err(err)?;
    // Stream counters after timing; avoid loading the query trace into peak RSS.
    let mut totals: std::collections::HashMap<(u64,u64,String), (usize, [f64;12])> = std::collections::HashMap::new();
    use std::io::BufRead;
    let names = ["graph", "full4_record", "rerank_record"];
    let fields = ["io_requests", "bytes_read", "shared_cache_hits", "query_cache_hits"];
    for line in std::io::BufReader::new(File::open(&trace_path).map_err(err)?).lines() {
        let q: serde_json::Value = serde_json::from_str(&line.map_err(err)?).map_err(err)?;
        let key = (q["search_width"].as_u64().unwrap(), q["beam_width"].as_u64().unwrap(), q["ablation"].as_str().unwrap().to_string());
        let entry = totals.entry(key).or_insert((0, [0.;12])); entry.0 += 1;
        for (i,name) in names.iter().enumerate() { for (j,field) in fields.iter().enumerate() {
            entry.1[i*4+j] += q[format!("{name}_{field}")].as_f64().unwrap_or(0.);
        }}
    }
    for row in artifact["summary_rows"].as_array_mut().ok_or("missing summary rows")? {
        let key = (row["search_width"].as_u64().unwrap(), row["beam_width"].as_u64().unwrap(), row["ablation"].as_str().unwrap().to_string());
        let (count, sum) = totals.get(&key).ok_or("missing I/O trace totals")?;
        for (i,name) in names.iter().enumerate() { for (j,field) in fields.iter().enumerate() {
            row[format!("{name}_{field}_per_query")] = serde_json::json!(sum[i*4+j] / *count as f64);
        }}
    }
    artifact["io_classification"] = serde_json::json!("graph_and_packed_records_by_request_stage_v1");
    artifact["ours_cache_allocation"]=serde_json::json!(cache_allocation);
    artifact["ours_record_cache_policy"]=serde_json::json!(if record_policy=="hot_dynamic" {record_cache::POLICY} else {record_policy});
    artifact["ours_record_cache_budget_bytes"]=serde_json::json!(record_budget);
    artifact["ours_record_cache_reserved_bytes"]=serde_json::json!(records.as_ref().map_or(0,|c|c.allocated));
    artifact["ours_graph_cache_bytes"]=serde_json::json!(graph_cache_bytes);
    artifact["ours_record_cache_stats"]=serde_json::json!(record_stats);
    for (field,flag) in [("ours_hot_profile_manifest","--ours-hot-profile-manifest"),
        ("ours_hot_profile_sha256","--ours-hot-profile-sha256"),("ours_hot_ranks_sha256","--ours-hot-ranks-sha256")] {
        artifact[field]=serde_json::json!(args.optional(flag).unwrap_or(""));
    }
    if profile.is_some() {artifact["throughput_comparable"]=serde_json::json!(false);}
    fs::write(result_path,serde_json::to_vec_pretty(&artifact).map_err(err)?).map_err(err)
}
...
fn run_search_phase(args: &Args, files: &IndexFiles, meta: &IndexMeta) -> Result<()> {
    ...
    match (meta.kind, storage_mode) {
        ...
        (CodecKind::Ours, "hybrid_disk") => {
            let formal_plan=formal_route_plan(args,&files.root)?;
            let pca_dir=args.optional("--pca-route-dir").map(Path::new);
            if let Some(plan)=&formal_plan {
                if args.optional("--adaptive-route-dir").is_some()
                    || args.optional("--locality-layout-dir").is_some()
                    || args.optional("--routing-hot-pages").is_some()
                    || args.optional("--routing-profile-output").is_some() {
                    return Err("formal PCA policy cannot mix with legacy diagnostic routing".into());
                }
                if (plan["mode"]=="pca1bit") != pca_dir.is_some() {
                    return Err("PCA assets must be supplied exactly when full 1-bit cannot fit; use the formal orchestrator".into());
                }
                if let Some(dir)=pca_dir {
                    if sha256(&dir.join("assets.json"))?!=args.text("--pca-assets-sha256")? {
                        return Err("PCA asset manifest hash mismatch".into());
                    }
                    let assets:serde_json::Value=serde_json::from_slice(&fs::read(dir.join("assets.json")).map_err(err)?).map_err(err)?;
                    for name in ["mean.bin","basis.bin","sidecar.bin","pca.json"] {
                        if assets["files"][name].as_str()!=Some(&sha256(&dir.join(name))?) {
                            return Err(format!("PCA asset hash mismatch: {name}"));
                        }
                    }
                }
                eprintln!("ours_route_plan={plan}");
            } else if pca_dir.is_some() {return Err("PCA assets require the formal automatic policy".into());}
            ...
            let routing_plan=if route.is_none() && formal_plan.is_none() {preflight_ours_routing(args,files,meta)?}else{RoutingPlan::Resident};
            let mut codec = OursResidentCodec::load_planned_pca(
                &files.ours_metadata, &files.ours_sidecar, route, routing_plan,pca_dir).map_err(err)?;
            if let (Some(plan),Some(pca))=(&formal_plan,&codec.pca) {
                if pca.k!=pca::field(plan,"dimension")? {return Err("PCA assets are not the highest fitting prefix".into());}
            }
            if let Some(reader)=codec.routing.as_mut() {
                let mut f=File::open(&files.ours_sidecar).map_err(err)?;let mut header=[0u8;24];f.read_exact(&mut header).map_err(err)?;
                let code_pages=usize::try_from(u64::from_le_bytes(header[8..16].try_into().unwrap()).checked_add(24).ok_or("codes overflow")?.div_ceil(4096)).map_err(err)?;
                let profile_pages=if args.optional("--routing-profile-output").is_some(){code_pages}else{0};
                reader.configure(args.optional("--routing-hot-pages").map(Path::new),profile_pages,code_pages).map_err(err)?;
            }else if args.optional("--routing-hot-pages").is_some() || args.optional("--routing-profile-output").is_some() {
                return Err("routing hot pages and profiling require paged storage".into());
            }
            if let Some(dir) = args.optional("--locality-layout-dir") {
                if route.is_some() || args.text("--layer")? != "05c"
            ...
            if let Some(dir) = args.optional("--ours-search-path-dir") {
                if args.optional("--parity-mode") != Some("external") {
                    return Err("search path diagnostic requires a disk-only process".into());
                }
                std::fs::create_dir_all(dir).map_err(err)?;
                codec.route_trace_dir = Some(std::path::PathBuf::from(dir));
            }
...
fn write_export_artifact(
    ...
    let document = format!(
        ...
        json_escape(if meta.kind==CodecKind::Ours && args.text("--layer")?=="05c" {OURS_SYSTEM_KERNEL} else {meta.kind.source_kernel()}),
        json_escape(if meta.kind==CodecKind::Ours && args.text("--layer")?=="05c" {"native_disk_system"} else {PORT_KIND}),
        json_escape(args.text("--implementation-fingerprint")?),
        json_escape(args.text("--native-binary-sha256")?),
        json_escape(args.text("--input-manifest-sha256")?),
        json_escape(args.text("--query-split-sha256")?),
...
fn run() -> Result<()> {
    let args = Args::parse()?;
    if let Some(dir)=args.optional("--pca-export") {return pca::export(Path::new(dir));}
    if let Some(index)=args.optional("--ours-route-plan") {
        let plan=formal_route_plan(&args,Path::new(index))?.ok_or("missing route policy")?;
        println!("{plan}");return Ok(());
    }
...
#[cfg(test)]
mod formal_width_tests {
    use super::*;
    #[test]
    fn explicit_widths_and_frozen_test_lock() {
        let mut args=Args {values: [("--search-widths","60,100,180"),("--layer","05c"),("--method","Ours-Disk"),("--phase","validation")]
            .into_iter().map(|(k,v)|(k.to_string(),v.to_string())).collect()};
        assert_eq!(formal_widths(&args).unwrap(),vec![60,100,180]);
        for value in ["0", "60,60", "100,60"] {
            args.values.insert("--search-widths".into(),value.into());
            assert!(formal_widths(&args).is_err());
        }
        args.values.insert("--search-widths".into(),"60,100,180".into());
        args.values.insert("--phase".into(),"test".into());
        let path=std::env::temp_dir().join(format!("qgraph-width-lock-{}.json",std::process::id()));
        fs::write(&path,r#"{"selected":{"Ours-Disk::hybrid_disk":{"search_widths":[60,100,180]}}}"#).unwrap();
        args.values.insert("--tuning-lock".into(),path.to_string_lossy().into_owned());
        assert_eq!(formal_widths(&args).unwrap(),vec![60,100,180]);
        args.values.insert("--search-widths".into(),"60,100".into());
        assert!(formal_widths(&args).is_err());
