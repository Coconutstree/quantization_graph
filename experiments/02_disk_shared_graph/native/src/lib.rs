...
pub struct QueryPageCache(*mut c_void);
unsafe impl Send for QueryPageCache {}

impl QueryPageCache {
    fn new() -> ANNResult<Self> {
        let ptr = unsafe { qgraph05_query_cache_new() };
        if ptr.is_null() { Err(direct_io_error()) } else { Ok(Self(ptr)) }
    }

    fn get(&mut self, file: u32, page: u64) -> Option<Box<[u8; PAGE_SIZE]>> {
        let mut bytes = std::mem::MaybeUninit::<[u8; PAGE_SIZE]>::uninit();
        // The bridge copies the entire page on a hit and leaves it untouched on a miss.
        if unsafe { qgraph05_query_cache_get(self.0, file, page, bytes.as_mut_ptr().cast()) } {
            Some(Box::new(unsafe { bytes.assume_init() }))
        } else {
            None
        }
    }

    fn put(&mut self, file: u32, page: u64, bytes: &[u8; PAGE_SIZE]) {
        unsafe { qgraph05_query_cache_put(self.0, file, page, bytes.as_ptr().cast()) };
    }

    fn allocated_bytes(&self) -> u64 {
        unsafe { qgraph05_query_cache_bytes(self.0) as u64 }
    }

    fn evictions(&self) -> u64 {
        unsafe { qgraph05_query_cache_evictions(self.0) }
    }
}

impl Drop for QueryPageCache {
    fn drop(&mut self) { unsafe { qgraph05_query_cache_delete(self.0) }; }
}
...
unsafe extern "C" {
    fn qgraph05_query_cache_new() -> *mut c_void;
    fn qgraph05_query_cache_delete(cache: *mut c_void);
    fn qgraph05_query_cache_get(cache: *mut c_void, file: u32, page: u64, out: *mut c_char) -> bool;
    fn qgraph05_query_cache_put(cache: *mut c_void, file: u32, page: u64, input: *const c_char);
    fn qgraph05_query_cache_bytes(cache: *mut c_void) -> usize;
    fn qgraph05_query_cache_evictions(cache: *mut c_void) -> u64;
...
pub struct IoStats {
    pub query_cache_allocated_bytes: u64,
...
pub struct DirectGraphReader {
    ...
    query_cache: Arc<Mutex<QueryPageCache>>,
}
...
impl DirectGraphReader {
    pub fn new(path: &Path, layout: GraphLayout) -> ANNResult<Self> {
        Ok(Self {
            ...
            query_cache: Arc::new(Mutex::new(QueryPageCache::new()?)),
        })
    }
    ...
    fn read_nodes(&mut self, ids: &[u32]) -> ANNResult<(Vec<Vec<u32>>, IoStats)> {
        ...
        let mut pages_for_batch = HashMap::new();
        let mut cache = self.query_cache.lock().map_err(|_| ann_error("query cache lock poisoned"))?;
        let evictions_before = cache.evictions();
        let missing = unique_pages
            .iter()
            .copied()
            .filter(|page| {
                if let Some(bytes) = cache.get(0, *page) {
                    pages_for_batch.insert(*page, bytes);
        ...
        if !missing.is_empty() {
            ...
            for (index, page_id) in missing.iter().copied().enumerate() {
                ...
                cache.put(0, page_id, &page);
                pages_for_batch.insert(page_id, page);
            }
        ...
        stats.query_cache_evictions = cache.evictions() - evictions_before;
        Ok((output, stats))
    }
}
...
mod tests {
    ...
    #[test]
    fn query_cache_bounds_eviction_and_file_isolation() {
        let mut cache = QueryPageCache::new().unwrap();
        assert!(cache.allocated_bytes() <= 4 * 1024 * 1024);
        cache.put(0, 7, &[1; PAGE_SIZE]);
        cache.put(1, 7, &[2; PAGE_SIZE]);
        assert_eq!(cache.get(0, 7).unwrap()[0], 1);
        assert_eq!(cache.get(1, 7).unwrap()[0], 2);
        for page in 0..3000 {
            cache.put(0, page, &[(page % 251) as u8; PAGE_SIZE]);
        }
        assert!(cache.evictions() > 0);
        assert!(cache.get(1, 7).is_none());
        assert_eq!(cache.get(0, 2999).unwrap()[0], (2999 % 251) as u8);
    }
    ...
    fn pq_codec_uses_official_kernel_and_roundtrips() {
        ...
        let before = codec.prepare(&[2.9, 3.2], Arc::new(Mutex::new(IoStats::default()))).unwrap();
        let expected = (0..3)
        ...
        let after = loaded.prepare(&[2.9, 3.2], Arc::new(Mutex::new(IoStats::default()))).unwrap();
        let actual = (0..3)
    ...
    fn sq_codec_uses_official_kernel_and_roundtrips() {
        ...
        let before = codec.prepare(&[1.1, 1.4], Arc::new(Mutex::new(IoStats::default()))).unwrap();
        let expected = (0..3)
            .map(|id| before.distance(id).unwrap())
            .collect::<Vec<_>>();
        ...
        let after = loaded.prepare(&[1.1, 1.4], Arc::new(Mutex::new(IoStats::default()))).unwrap();
        let actual = (0..3)
            .map(|id| after.distance(id).unwrap())
            .collect::<Vec<_>>();
        assert_eq!(actual, expected);
    ...
    fn saq_codec_reconstructs_transform_and_roundtrips_codes() {
        ...
        let before = codec.prepare(&[1.1, 1.4, 1.6, 1.8], Arc::new(Mutex::new(IoStats::default()))).unwrap();
        let expected = (0..3)
            .map(|id| before.distance(id).unwrap())
            .collect::<Vec<_>>();
        ...
        let after = loaded.prepare(&[1.1, 1.4, 1.6, 1.8], Arc::new(Mutex::new(IoStats::default()))).unwrap();
        let actual = (0..3)
            .map(|id| after.distance(id).unwrap())
            .collect::<Vec<_>>();
        assert_eq!(actual, expected);
    ...
    #[test]
    fn graph_batch_larger_than_cache_keeps_every_requested_page() {
        let tmp = TestDir::new();
        let path = tmp.path().join("large.pages");
        let count = 1300;
        let mut file = File::create(&path).unwrap();
        for id in 0..count {
            let mut page = [0; PAGE_SIZE];
            page[..4].copy_from_slice(&1_u32.to_le_bytes());
            page[4..8].copy_from_slice(&(((id + 1) % count) as u32).to_le_bytes());
            file.write_all(&page).unwrap();
        }
        drop(file);
        let layout = GraphLayout { node_count: count, max_degree: 1, start_point: 0,
            additional_points: 0, record_bytes: 8, nodes_per_page: 1, page_count: count };
        let mut reader = DirectGraphReader::new(&path, layout).unwrap();
        let ids = (0..count as u32).collect::<Vec<_>>();
        let (rows, stats) = reader.read_nodes(&ids).unwrap();
        for (id, row) in rows.iter().enumerate() {
            assert_eq!(row, &[((id + 1) % count) as u32]);
        }
        assert!(stats.query_cache_evictions > 0);
        assert!(stats.query_cache_allocated_bytes <= 4 * 1024 * 1024);
        assert_eq!(stats.bytes_read, count as u64 * PAGE_SIZE as u64);
    }
}
