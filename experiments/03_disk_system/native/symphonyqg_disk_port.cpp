...
// Compare the complete serialized official index with its padded disk export.
// This covers raw vectors, codes, factors, adjacency, entry point and rotation.
void verify_export(const fs::path& raw, const fs::path& root, const Meta& meta) {
    std::ifstream original(raw, std::ios::binary);
    std::ifstream pages(root / "node_rows.pages", std::ios::binary);
    std::ifstream rotation(root / "rotator.bin", std::ios::binary);
    std::uint32_t entry = 0;
    original.read(reinterpret_cast<char*>(&entry), sizeof(entry));
    if (!original || entry != meta.entry_point) throw std::runtime_error("export entry point mismatch");
    std::vector<char> expected(meta.row_bytes), actual(meta.pages_per_row * kPage);
    for (std::size_t id = 0; id < meta.n; ++id) {
        original.read(expected.data(), expected.size());
        pages.read(actual.data(), actual.size());
        if (!original || !pages || !std::equal(expected.begin(), expected.end(), actual.begin()) ||
            !std::all_of(actual.begin() + meta.row_bytes, actual.end(), [](char c) { return c == 0; }))
            throw std::runtime_error("export row/padding mismatch at node " + std::to_string(id));
    }
    if (pages.peek() != std::char_traits<char>::eof()) throw std::runtime_error("extra export rows");
    const std::size_t bytes = meta.padded_dim * sizeof(float);
    expected.resize(bytes);
    actual.resize(((bytes + kPage - 1) / kPage) * kPage);
    original.read(expected.data(), expected.size());
    rotation.read(actual.data(), actual.size());
    if (!original || !rotation || !std::equal(expected.begin(), expected.end(), actual.begin()) ||
        !std::all_of(actual.begin() + bytes, actual.end(), [](char c) { return c == 0; }) ||
        original.peek() != std::char_traits<char>::eof() || rotation.peek() != std::char_traits<char>::eof())
        throw std::runtime_error("export rotator/length mismatch");
}
...
bool reusable_index(const fs::path& root, const Args& args) {
    ...
    try {
        std::ifstream proof(root / "export.sha256");
        std::string version, official_hash, rows_hash, rotation_hash, meta_hash;
        if (!(proof >> version >> official_hash >> rows_hash >> rotation_hash >> meta_hash) ||
            version != "symphonyqg_export_v1" || official_hash.size() != 64 ||
            rows_hash != sha256(rows_path) || rotation_hash != sha256(rotator_path) ||
            meta_hash != sha256(meta_path)) return false;
...
void export_index(const Args& args) {
    ...
    if (fs::exists(root / "index.meta")) {
        throw std::runtime_error("SymphonyQG index lacks valid export provenance; use a new index directory");
    }
    ...
    // The pinned official builder does not export this counter. Zero is the
    // legacy metadata sentinel for unavailable, not a measured operation count.
    meta.build_distance_computations = 0;
    ...
    verify_export(raw, root, meta);
    std::ofstream proof(root / "export.sha256");
    proof << "symphonyqg_export_v1\n" << sha256(raw) << '\n'
          << sha256(root / "node_rows.pages") << '\n'
          << sha256(root / "rotator.bin") << '\n'
          << sha256(root / "index.meta") << '\n';
    proof.flush();
    if (!proof) throw std::runtime_error("failed to write export provenance");
    fs::remove(raw);
}

struct QueryStats {
    std::uint64_t shared_cache_hits=0, shared_cache_misses=0, shared_cache_bytes=0;
...
struct RowReader {
    ...
    qgraph05::SharedPageCache* shared;

    RowReader(const fs::path& path, const Meta& meta_ref, QueryStats* stat_ref, qgraph05::SharedPageCache* shared_ref=nullptr)
            : reader(path), meta(meta_ref), stats(stat_ref), shared(shared_ref) {}
    ...
    const char* row(std::uint32_t id) {
        ...
        stats->shared_cache_bytes=shared->allocated_bytes();
        const auto first=static_cast<std::uint64_t>(id)*meta.pages_per_row;
        current_row.resize(meta.pages_per_row*kPage);
        std::vector<std::uint64_t> missing;
        for(std::size_t i=0;i<meta.pages_per_row;++i) {
            auto* target=current_row.data()+i*kPage;
            if(cache.get(0,first+i,target)) continue;
            if(shared->get(first+i,target)) {
                ++stats->shared_cache_hits;
                cache.put(0,first+i,target);
            } else { ++stats->shared_cache_misses; missing.push_back(first+i); }
        }
        if(!missing.empty()) {
            const auto start=Clock::now();
            auto batch=reader.read_pages(missing);
            stats->io_wait_us+=std::chrono::duration<double,std::micro>(Clock::now()-start).count();
            stats->bytes_read+=batch.stats().bytes_read;
            stats->io_requests+=batch.stats().submitted_requests;
            stats->coalesced+=batch.stats().coalesced_requests;
            stats->duplicates+=batch.stats().duplicate_pages_removed;
            for(auto page:missing) {
                auto* target=current_row.data()+(page-first)*kPage;
                std::memcpy(target,batch.page(page).data(),kPage);
                cache.put(0,page,target); shared->put(page,target);
            }
        }
...
void write_artifact(
    ...
    out << "  \"timing_semantics\": \"search_wall_excludes_recall_evaluation\",\n";
    out << "  \"unavailable_metrics\": [\"distance_compute_us\", \"queue_compute_us\"],\n";
    ...
    out << "  \"shared_page_cache_policy\": \"index_local_16shard_lru_v1\",\n";
    out << "  \"cache_bytes\": " << (stats.empty()?0:stats[0].shared_cache_bytes) << ",\n";
    out << "  \"cache_nodes\": 0,\n";
    ...
    out << "  \"summary_rows\": [{\n";
    out << "    \"shared_cache_bytes\": " << (stats.empty()?0:stats[0].shared_cache_bytes) << ",\n";
    ...
    out << "    \"distance_compute_us\": " << "null,\n";
    out << "    \"query_prep_us\": " << (prep / n) << ",\n";
    out << "    \"queue_compute_us\": " << "null,\n";
    out << "    \"traversal_wall_us\": " << (traverse / n) << ",\n";
...
void run_search_single(const Args& args) {
    ...
    // Reserve the already loaded process plus per-worker query/cache/AIO/queue headroom.
    // Whole-process RSS admission remains authoritative; this is a conservative planner.
    const double budget_gib=std::stod(args.require("search-dram-budget-gib"));
    if (!std::isfinite(budget_gib) || budget_gib<=0 || budget_gib>1048576)
        throw std::runtime_error("invalid memory budget");
    const auto budget=static_cast<std::uint64_t>(budget_gib*(1ULL<<30));
    const auto queue_reserve=std::min<std::uint64_t>(meta.n/10, std::uint64_t(ef)*ef)*32 + std::uint64_t(ef)*128;
    std::ifstream statm("/proc/self/statm");
    std::uint64_t virtual_pages=0, resident_pages=0;
    if(!(statm>>virtual_pages>>resident_pages)) throw std::runtime_error("cannot read current RSS");
    const auto reserve=resident_pages*static_cast<std::uint64_t>(sysconf(_SC_PAGESIZE))+(64ULL<<20)+workers*((16ULL<<20)+queue_reserve);
    if(reserve>budget) throw std::runtime_error("search reserve exceeds memory budget");
    std::uint64_t cache_budget=args.require("cache-mode")=="c0" ? 0 : budget-reserve;
    if(args.values.count("shared-page-cache-bytes")) {
        const auto text=args.require("shared-page-cache-bytes");
        if(text.empty() || text.find_first_not_of("0123456789")!=std::string::npos)
            throw std::runtime_error("shared-page-cache-bytes must be a nonnegative integer");
        const auto requested=std::stoull(text);
        if(requested>cache_budget) throw std::runtime_error("shared cache exceeds remaining budget or c0 policy");
        cache_budget=requested;
    }
    // Never reserve more than the full index plus cache metadata.
    cache_budget=std::min<std::uint64_t>(cache_budget, fs::file_size(root/"node_rows.pages")*2+(1ULL<<20));
    ...
    auto run_query = [&](std::size_t qid, bool record) {
        ...
        RowReader reader(root / "node_rows.pages", meta, &local, &shared);
        auto ids = search_one(
                reader,
                meta,
                q_obj,
        ...
        local.sort_us = 0.0; // No separate rerank stage; Recall is evaluated after timing.
...
#pragma omp parallel for schedule(dynamic) num_threads(workers)
    ...
    // Recall evaluation and resource sampling are outside the throughput interval.
    const auto measured_rss = qgraph05::measured_peak_rss();
    for (std::size_t qid = 0; qid < queries.rows; ++qid) {
        stats[qid].recall = recall_at_10(top10[qid], gt[qid]);
        stats[qid].peak_rss_bytes = measured_rss;
    }
    ...
    for (std::size_t qid = 0; qid < queries.rows; ++qid) {
        ...
        trace << "{"
              ...
              << ",\"cache_nodes\":0,\"resident_bytes\":0,\"cache_bytes\":" << s.shared_cache_bytes << ",\"peak_rss_bytes\":" << s.peak_rss_bytes << ","
              ...
              << ",\"queue_compute_us\":null"
              << ",\"io_wait_us\":" << s.io_wait_us << ",\"distance_compute_us\":null,\"traversal_wall_us\":" << s.traverse_us
              << ",\"rerank_us\":" << s.sort_us
              ...
              << ",\"shared_cache_hits\":" << s.shared_cache_hits << ",\"shared_cache_misses\":" << s.shared_cache_misses << ","
    ...
    trace.close();
    if (!qgraph05::reference_mmap_enabled() && measured_rss>budget) throw std::runtime_error("whole-process peak RSS exceeds budget");
    write_artifact(
            args,
            meta,
