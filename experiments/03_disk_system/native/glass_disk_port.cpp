...
void run_search_single(const Args& args) {
    ...
    auto run_query = [&](std::size_t qid, bool record) {
        ...
        local.latency_us = std::chrono::duration<double, std::micro>(q2 - q0).count();
...
#pragma omp parallel for schedule(dynamic) num_threads(workers)
    ...
    for (std::size_t qid = 0; qid < queries.rows; ++qid) {
        stats[qid].recall = recall_at_10(ids[qid], gt[qid]);
...
int main(int argc, char** argv) {
    ...
    trace << std::setprecision(17);
            ...
            // Search and reference validation must never rebuild their inputs.
            if (!reusable_index(fs::path(args.require("disk-index-dir")), args)) {
                throw std::runtime_error("Glass index provenance/geometry mismatch; explicit export is required before search");
            }
            run_search(args);
