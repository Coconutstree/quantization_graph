    ...
    Factory: diskann_disk::search::traits::VertexProviderFactory<Data> + Sync,
{
    for &query_id in order.iter().take(warmup_queries.min(order.len())) {
        let _cache_scope = query_cache::QueryScope::new();
        searcher.search(
            &queries[query_id],
            K,
            width as u32,
    ...
    let mut runs = pool.install(|| {
        order
            ...
            .map(|&query_id| -> Result<QueryRun> {
                ...
                let cache_scope = query_cache::QueryScope::new();
...
fn summary_row(
    ...
    let mut latencies = batch
        ...
        .collect::<Vec<_>>();
    let io_requests = mean(&batch.runs, |run| run.cache_stats.requests as f64);
    Ok(json!({
        ...
        "sectors_4k_per_query": mean(&batch.runs, |run| run.cache_stats.bytes_read as f64 / PAGE_SIZE as f64),
        "bytes_read_per_query": mean(&batch.runs, |run| run.cache_stats.bytes_read as f64),
...
fn formal_widths(args: &Args) -> Result<Vec<usize>> {
    ...
    if let Some(value) = args.optional("--measurement-width") {
        ...
        if !widths.contains(&width) { bail!("measurement width outside frozen grid"); }
        return Ok(vec![width]);
    }
    Ok(widths)
}
...
fn validation_beams(args: &Args) -> Result<Vec<usize>> {
    if let Some(value) = args.optional("--fixed-beam") {
        let beam: usize = value.parse().context("invalid --fixed-beam")?;
        if beam == 0 { bail!("--fixed-beam must be positive"); }
        return Ok(vec![beam]);
    }
    if std::env::var("QG05_FAST").ok().as_deref() == Some("1") {
        return Ok(vec![1]);
...
fn selected_test_beam(args: &Args) -> Result<usize> {
    ...
    if let Some(value) = args.optional("--fixed-beam") {
        if config != format!("beam{value}") { bail!("fixed beam differs from tuning lock"); }
    }
    config
        .strip_prefix("beam")
...
fn cache_plan(args: &Args, meta: &IndexMeta) -> Result<(usize, usize, usize)> {
    ...
    let cache_nodes = match args.text("--cache-mode")? {
        ...
        // not "10% of the base points".  The old `base_count / 10` cap kept only ~344 MB of
        // the 4 GiB budget in the node cache while the rest of the budget stayed unused.
        "standard" => ((budget - fixed) / meta.cached_node_bytes).min(meta.base_count),
...
fn write_trace_rows(
    ...
    for run in &batch.runs {
        let io_requests = run.cache_stats.requests;
        let bytes_read = run.cache_stats.bytes_read;
        let row = json!({
            "layer": LAYER,
            ...
            "query_cache_hits": run.cache_stats.hits,
            "query_cache_misses": run.cache_stats.misses,
            "query_cache_evictions": run.cache_stats.evictions,
            "query_cache_allocated_bytes": run.cache_stats.allocated_bytes,
        });
...
fn run_search(args: &Args, files: &IndexFiles, meta: &IndexMeta) -> Result<()> {
    ...
    let independent_warmup = if let Some(path) = args.optional("--warmup-query") {
        if sha256(Path::new(path))? != args.text("--warmup-query-sha256")? {
            bail!("independent warmup hash mismatch");
        }
        let q = read_fvecs(Path::new(path))?;
        if q.len() < args.number::<usize>("--warmup-queries")? || q.iter().any(|v| v.len() != meta.dimension) {
            bail!("independent warmup shape/count mismatch");
        }
        Some(q)
    } else { None };
    ...
    // Fairness: every method warms up the same 100 queries with the same 32-way
    // parallelism (Ours, Glass and Symphony all warm up across `workers` threads).
    // Warmup stays outside the timed interval; this only makes the shared
    // pre-measurement cache state comparable across methods.
    ...
    for &beam in &beams {
        for &width in &widths {
            ...
            let direct_factory = DiskVertexProviderFactory::<AdHoc<f32>, cached_reader::Factory>::new(
                cached_reader::Factory(files.disk_index.to_string_lossy().into_owned()),
                caching_strategy(cache_nodes),
            )?;
            let direct_searcher = DiskIndexSearcher::<AdHoc<f32>, _>::new(
                workers,
                usize::MAX, // Width-only protocol: no additional traversal I/O cap.
                &index_reader,
                direct_factory,
                Metric::L2,
                None,
            )?;
            let warm_vectors = independent_warmup.as_ref().unwrap_or(&queries);
            let warmup_ids: Vec<usize> = if independent_warmup.is_some() {
                (0..warmup.min(warm_vectors.len())).collect()
            } else { order.iter().take(warmup.min(order.len())).copied().collect() };
            warmup_pool.install(|| {
                warmup_ids.par_iter().try_for_each(|&query_id| -> Result<()> {
                    let _cache_scope = query_cache::QueryScope::new();
                    direct_searcher.search(
                        &warm_vectors[query_id],
                        K,
                        width as u32,
                        Some(beam),
                        SearchMode::graph(),
            ...
            reset_peak_rss()?;
            eprintln!("DiskANN width={width}: warmup complete; starting disk batch");
            ...
            eprintln!("DiskANN width={width}: disk batch complete; starting memory parity");

            // Reference materialization must not enter a memory-capped disk measurement.
            ...
            eprintln!("DiskANN width={width}: memory parity complete; writing trace");
            write_trace_rows(
                &mut trace,
                args,
                &direct,
            ...
            trace.flush()?;
            eprintln!("DiskANN width={width}: trace saved; releasing search resources");
        }
    }
    trace.flush()?;
    if parity.count > 0 && (parity.max_recall_delta > 1e-3
    ...
    write_json(
        ...
        &json!({
            ...
            "query_cache_policy": "4k_lru_per_query_v1",
            "query_cache_budget_per_worker_bytes": query_cache::BUDGET,
            "memory_accounting_complete": false,
            "cache_bytes": cache_bytes,
            "cache_nodes": cache_nodes,
            ...
            "page_size": PAGE_SIZE,
            "formal_ready": false,
            "implementation_parity": if parity.count > 0 { "passed" } else { "not_verified" },
            "parity": {
            ...
            "measurement_notes": {
                ...
                "bytes_read": "actual cache-miss read buffers submitted to LinuxAlignedFileReader; cache hits submit no I/O",
            },
