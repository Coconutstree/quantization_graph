#!/usr/bin/env python3
"""Small integration test for the real 05C Glass-NSG disk port."""
...
def main() -> int:
    ...
    binary = repo / "build/formal_local/05_disk_system_fair/qgraph05_glass_disk_port"
    if not binary.exists():
        raise SystemExit(f"build qgraph05_glass_disk_port first: {binary}")
    with tempfile.TemporaryDirectory(prefix="qgraph05_glass_disk_") as tmp:
        root = Path(tmp)
        dataset_root = root / "data/tiny"
        dataset_root.mkdir(parents=True)
        rng = random.Random(20260813)
        count, dim, query_count = 256, 16, 4
        ...
        command = [
            ...
            "--direct-io", "required",
            "--native-aio", "required",
            "--implementation-fingerprint", "glass-nsg-sq4u-disk-native-integration",
        ...
        artifact = validate_artifact(
            result,
            spec=SPECS_BY_KEY["05c:Glass-NSG-DiskPort"],
            expected={
                "dataset": "tiny",
                "phase": "validation",
                "run_id": "glass_disk_integration",
                "repeat_id": 0,
                "workers": 1,
                "storage_mode": "hybrid_disk",
                "cache_mode": "standard",
                "implementation_fingerprint": "glass-nsg-sq4u-disk-native-integration",
        ...
        print(
            "PASS Glass-NSG-DiskPort O_DIRECT native integration "
