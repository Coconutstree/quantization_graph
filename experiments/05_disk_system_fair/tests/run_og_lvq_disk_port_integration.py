#!/usr/bin/env python3
"""Small integration test for the 05C OG-LVQ disk port."""
...
def main() -> int:
    ...
    binary = repo / "build/formal_local/05_disk_system_fair/qgraph05_og_lvq_disk_port"
    if not binary.exists():
        raise SystemExit(f"build qgraph05_og_lvq_disk_port first: {binary}")
    with tempfile.TemporaryDirectory(prefix="qgraph05_og_lvq_disk_") as tmp:
        root = Path(tmp)
        ...
        command = [
            ...
            "--run-id", "og_lvq_disk_integration",
            "--repeat-id", "0",
            "--workers", "1",
            "--warmup-queries", "0",
            ...
            "--direct-io", "required",
            "--native-aio", "required",
            "--implementation-fingerprint", "og-lvq-svs-layout-disk-native-integration",
        ...
        artifact = validate_artifact(
            result,
            spec=SPECS_BY_KEY["05c:OG-LVQ-DiskPort"],
            expected={
                "dataset": "tiny",
                "phase": "validation",
                "run_id": "og_lvq_disk_integration",
                "repeat_id": 0,
                "workers": 1,
                "storage_mode": "hybrid_disk",
                "cache_mode": "standard",
                "implementation_fingerprint": "og-lvq-svs-layout-disk-native-integration",
        ...
        print(
            "PASS OG-LVQ-DiskPort O_DIRECT native integration "
