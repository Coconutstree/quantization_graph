...
SUITE = REPO_ROOT / "experiments" / "05_disk_system_fair" / "run_disk_suite.py"
PLOTTER = REPO_ROOT / "experiments" / "05_disk_system_fair" / "plot_05_disk_suite.py"
...
LAYER_METHODS = {
    ...
    "05c": (
        "Ours-Disk,SymphonyQG-DiskPort,OG-LVQ-DiskPort,"
...
def build_graphs(
    ...
    dest_root = REPO_ROOT / "results" / dataset / "indexes" / "02_diskann_fair"
    shared_src = (
        build_out
        / dataset
        / "indexes"
...
def run_round(
    ...
    for plot_layers, methods in (
        ...
        plot_cmd = [
            sys.executable,
            str(PLOTTER),
            "--run-root", str(args.out_root.resolve() / "runs" / run_id),
            "--layers", plot_layers,
            "--datasets", args.dataset,
            "--workers", str(workers),
...
def read_third_party_build_stats(dataset: str) -> list[dict[str, str]]:
    base = REPO_ROOT / "results" / "03_system_fair" / dataset / "indexes"
    stats = []
...
def read_export_sidecars(
    ...
    run_root = args.out_root.resolve() / "runs" / run_id
    stats = []
    for layer, layer_dir in LAYER_DIRS.items():
        artifacts = run_root / layer_dir / args.dataset / "artifacts" / "export"
        for sidecar in sorted(artifacts.glob("*.build_stats.json")):
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--out-root",
        type=Path,
        default=REPO_ROOT / "results" / "disk_environment" / ".test_runs",
    )
    parser.add_argument(
        "--disk-root",
        type=Path,
