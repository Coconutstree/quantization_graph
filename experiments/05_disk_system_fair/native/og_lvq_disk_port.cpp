...
constexpr const char* kSourceKernel = "official SVS LVQ4 distance kernel";
...
struct DiskRows {
    ...
    Meta meta;
    QueryStats* stats;
    std::unordered_map<std::uint32_t, std::vector<char>> graph_cache;
    std::unordered_map<std::uint32_t, std::vector<char>> lvq_cache;
    ...
    const char* read_row(
        ...
        if (it == cache.end()) {
            ...
            std::memcpy(row.data(), view.data() + offset, record_bytes);
            ...
            it = cache.emplace(id, std::move(row)).first;
        }
...
struct Candidate { std::uint32_t id = 0; float distance = 0.0f; bool expanded = false; };

void insert_candidate(std::vector<Candidate>& pool, std::unordered_set<std::uint32_t>& seen,
                      std::uint32_t id, float distance, int ef, std::size_t n) {
...
std::vector<int32_t> search_one(DiskRows& rows, const Centroids& centroids, const float* query,
    ...
    insert_candidate(pool, seen, rows.meta.entry_point,
                     lvq_distance(rows.lvq(rows.meta.entry_point), rows.meta, centroids, query),
                     ef, rows.meta.n);
    while (true) {
        auto it = std::find_if(pool.begin(), pool.end(), [](const Candidate& c) { return !c.expanded; });
        ...
        for (std::size_t i = 0; i < degree; ++i) {
            ...
            if (v >= rows.meta.n || seen.count(v)) continue;
            const float d = lvq_distance(rows.lvq(v), rows.meta, centroids, query);
            ++stats.distance_calls;
            insert_candidate(pool, seen, v, d, ef, rows.meta.n);
        }
    }
    std::sort(pool.begin(), pool.end(), [](const Candidate& a, const Candidate& b) {
        return a.distance < b.distance || (a.distance == b.distance && a.id < b.id);
