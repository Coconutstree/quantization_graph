"""Render evidence-backed baseline audit after the four isolated diagnostic runs."""
import json
from pathlib import Path
from protocol import ROOT,OUT,REFERENCE
D=ROOT/'results/04_ours_memory_budget/baseline_diagnosis_20260919'

def main():
 m=json.loads((D/'diagnosis_metrics.json').read_text());assert len(m['runs'])==4
 for row in m['runs']:
  p=D/row['run'];assert json.loads((p/'parity.json').read_text())['passed'];assert json.loads((p/'memory_measurement.json').read_text())['user_address_space_budget_passed']
 old={r['search_width']:r for r in json.loads((REFERENCE/'result.json').read_text())['summary_rows']}
 new={r['search_width']:r for r in json.loads((OUT/'baseline/result.json').read_text())['summary_rows']}
 lines=['# GIST 基线QPS异常：历史原程序与新版交错复测','',
 ...
 f"| 历史记录 | 原历史程序 | {old[100]['qps']:.2f} | {old[100]['io_wait_us']/1000:.2f} | 未记录 | 未记录 |",
 f"| v2原记录 | 新版baseline | {new[100]['qps']:.2f} | {new[100]['io_wait_us']/1000:.2f} | 未记录 | 未记录 |"]
 for row in m['runs']:
  label='历史原二进制' if row['run'].endswith('old') else '新版二进制'
  lines.append(f"| {row['run']} | {label} | {row['qps']:.2f} | {row['io_wait_ms_per_query']:.2f} | {row['sda_read_await_ms']:.2f} | {row['host_cpu_idle_percent']:.1f}% |")
 lines += ['',f"旧程序两次平均 **{m['old_mean_qps']:.2f} QPS**，新版两次平均 **{m['new_mean_qps']:.2f} QPS**，新版/旧版为 **{m['new_over_old']:.3f}**。这个差异无法解释历史与本轮约11倍的差距；但每种仅两次，不用于证明零开销或统计显著差异。",'',
 '### 配置相同的边界','', '搜索参数、输入内容、页布局、每查询4 MiB LRU、测量器环境及地址空间上限已逐项比对；但不能称所有条件完全一致。历史和v2正式运行均为40档完整扫描，本次ABBA仅L100。历史L100之前已跑过27档L=10–90，这可能预热控制器/设备缓存；每档100条查询预热不等价于此前完整扫描的存储状态。ABBA证明同一当前诊断条件下新旧程序接近，尚不能单独区分历史环境变化与扫描前序引起的缓存状态差异。历史原二进制完整40档对照现已获授权并启动，见[独立复现报告](results/04_ours_memory_budget/historical_full_replay_20260919/report.md)及[实时状态](results/04_ours_memory_budget/historical_full_replay_20260919/state.json)。','', '此外新版结果多出timing_semantics=search_wall_excludes_recall_evaluation，旧结果没有该字段；不能仅由字段缺失认定计时实现相同或不同。不过同一旧二进制的历史/当前差异不涉及新旧版本计时定义变化。CPU允许范围0–159和NUMA节点范围0–3一致，但不代表每个线程实际调度到相同核心或相同NUMA内存放置。','', '### 完整顺序复现的阶段性更新','', '完整扫描已通过L100及其后的L140/180/220。L100前27档均按原顺序执行，但平均查询延迟仍为3186.86 ms（历史289.49 ms），读取路径等待3172.80 ms（历史256.80 ms）。因此单测L100缺少前序扫描不足以解释差距；所有历史缓存状态因素尚未排除。任务尚未完成，准确QPS等待原生result.json，不用延迟倒数替代。详见[阶段性复盘](results/04_ours_memory_budget/historical_full_replay_20260919/interim_review.md)。','', '## 2. 已从“读取路径计时”进一步定位到实际块设备等待','',
 ...
 '### baseline封装与计时专项排查','', 'QPS在原生程序内按完成查询数/搜索墙钟时间计算，不是Python测量器重算。历史40档计时合计316.41秒，外部进程376.39秒；v2分别3431.13秒和3897.33秒。L100的QPS×平均查询延迟分别约31.64与31.62，未发现线程数或微秒/秒换算导致十倍读数错误；这些只是计时一致性检查，不证明所有计时路径没有问题。','', '已追加[启动封装对照](results/04_ours_memory_budget/launch_path_diagnosis_20260919/report.md)：等待当前历史40档复现完成后，串行直接启动新版/旧版L100，保留2 GiB及原查询参数，去掉/proc周期轮询和主机遥测。该项执行前不能声称已排除测量器开销。','', '## 5. 对缓存实验结果的影响','',
 ...
 (ROOT/'ours_memory_budget_baseline_audit.md').write_text('\n'.join(lines))
 review=json.loads((OUT/'performance_review.json').read_text());review['diagnosis']='Historical executable reproduces low QPS in current environment; current bottleneck localized to block-device/AIO completion waits; historical storage-state difference remains unresolved.';review['abba']=m
 (OUT/'performance_review.json').write_text(json.dumps(review,indent=2))
 print('updated audit with four validated ABBA runs')
