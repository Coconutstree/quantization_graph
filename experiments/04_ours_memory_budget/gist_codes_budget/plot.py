...
import numpy as np
ROOT=Path(__file__).resolve().parents[3];OUT=ROOT/'results/04_ours_memory_budget/gist_codes_budget'
...
def main():
 ...
 axes[1].set(title='b  Fixed search widths',xlabel='Codes/cache capacity (discrete settings)',ylabel='QPS')
 ...
 fig.suptitle('GIST · beam1 · 32 workers · 100 test queries\nTwo runs; process cap fixed at 2 GiB; T = 122.070313 MiB; factors outside quota',fontsize=9)
 folder=OUT/'figures';folder.mkdir(exist_ok=True)
 fig.savefig(folder/'codes_budget_curves.png',dpi=300)
 fig.savefig(folder/'codes_budget_curves.pdf')
 fig.savefig(folder/'codes_budget_curves.svg')
 plt.close(fig)
 contract=dict(backend='python',archetype='quantitative grid',claim='Codes capacity alone selects paging versus resident codes; factors and runtime memory outside quota',panels={'a':'Recall/QPS for each data quota and resident reference','b':'QPS against discrete quota settings preserving T minus one byte versus T'},input_rows=48,excluded_rows=0,repeats=2,center='harmonic mean QPS (equal measured query count)',uncertainty='two-run min/max, not confidence interval',process_cap_mib=2048,data_threshold_bytes=exp['resident_codes_threshold_bytes'],source_sha256=hashlib.sha256(source.read_bytes()).hexdigest(),exports=['PNG report preview 300 dpi','PDF editable','SVG editable'],size_mm=[183,120])
