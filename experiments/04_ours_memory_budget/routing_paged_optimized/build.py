...
def build():
    ...
    binary=WORK/'ours_routing_paged_optimized'
    shutil.copy2(ROOT/'src/graph_core/target/release/qgraph05_shared_graph_port',binary)
    sources=[]
    for folder,patterns in [('experiments/02_disk_shared_graph/native',['**/*.rs','*.cpp']),('src/graph_core',['src/*.rs','native/*.cpp','Cargo.*','build.rs']),('src/disk_bench/native',['*.cpp','*.hpp']),('Ours/core/hnswlib',['*.h']),('experiments/04_ours_memory_budget/routing_paged',['*.py','*.cpp']),('experiments/04_ours_memory_budget/routing_paged_optimized',['*.py','*.cpp'])]:
