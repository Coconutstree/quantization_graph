...
// GIST / 32 workers: validated against 64 pages on independent test800.
const DEFAULT_INFLIGHT:usize=256;
fn inflight_limit(value:Option<&str>,capacity:usize)->Result<usize>{
 let limit=value.unwrap_or("256").parse::<usize>().map_err(err)?;
 if limit==0 || limit>DEFAULT_INFLIGHT{return Err("routing inflight must be 1..256".into());}
 Ok(limit.min(capacity))
}
#[derive(Clone,Debug)]
...
pub fn choose(b:usize,fixed:usize,reserve:usize,c:usize,f:usize,scratch:usize,min_record:usize,mode:&str,cap:Option<usize>)->Result<Plan>{
 ...
 if resident {
  ...
  Ok(Plan{budget:b,routing:RoutingPlan::Paged(PagingPlan{clock:true,resident_factors:true,capacity,inflight:DEFAULT_INFLIGHT.min(capacity),reserved_bytes:reserved}),optional:0,policy:"paged".into(),expected_peak:sum(&[fixed,f,scratch,reserved])?})
 }
}
pub fn routing(args:&Args,codes:u64,factors:u64,centroid:u64,stride:u64,workers:u64,_mapping:u64)->Result<RoutingPlan>{
 ...
 // Shared service concurrency is independent of the 64-candidate worker scratch.
 inflight_limit(args.optional("--routing-max-inflight-pages"),1)?;
 let result=choose(b,fixed,reserve,c,f,scratch,min_record,mode,cap).and_then(|mut p|{
  if let RoutingPlan::Paged(ref mut page)=p.routing{
   page.inflight=inflight_limit(args.optional("--routing-max-inflight-pages"),page.capacity)?;
  }
  Ok(p)
 });
...
#[cfg(test)]mod tests{
 use super::*;
 #[test]fn shared_concurrency_default_override_and_small_cache(){
  assert_eq!(inflight_limit(None,4096).unwrap(),256);
  assert_eq!(inflight_limit(Some("64"),4096).unwrap(),64);
  assert_eq!(inflight_limit(None,83).unwrap(),83);
  assert_eq!(inflight_limit(Some("64"),1).unwrap(),1);
  for value in ["0","257","invalid"]{assert!(inflight_limit(Some(value),4096).is_err());}
  let p=choose(538*MIB,426*MIB,64*MIB,128000000,20000000,2703360,4001177,"auto",None).unwrap();
  if let RoutingPlan::Paged(page)=p.routing{assert_eq!(page.inflight,256);}else{panic!()}
  assert!(p.expected_peak+64*MIB<=538*MIB);
 }
