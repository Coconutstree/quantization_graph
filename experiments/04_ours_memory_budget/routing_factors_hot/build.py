...
def build():
    ...
    sources=[]
    for folder,patterns in [('experiments/02_disk_shared_graph/native',['**/*.rs','*.cpp']),('src/graph_core',['src/*.rs','native/*.cpp','Cargo.*','build.rs']),('src/disk_bench/native',['*.cpp','*.hpp']),('Ours/core/hnswlib',['*.h']),('experiments/04_ours_memory_budget/routing_paged',['*.py','*.cpp']),('experiments/04_ours_memory_budget/routing_paged_optimized',['*.py','*.cpp']),('experiments/04_ours_memory_budget/routing_factors_hot',['*.py','*.cpp'])]:
