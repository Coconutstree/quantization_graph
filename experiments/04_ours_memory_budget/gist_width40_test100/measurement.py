        ...
        try:
            while True:
                try:
                    ...
                    sample = dict(unix=time.time(), width=width, phase=phase)
            ...
            if process.returncode is None:
                ...
                _, status, usage = os.wait4(process.pid, 0)
                process.returncode = os.waitstatus_to_exitcode(status)
            evidence["interrupted"] = type(exc).__name__
    with (folder / "rss_samples.jsonl").open('w') as stream:
        for sample in samples:
            stream.write(json.dumps(sample) + '\n')
    evidence['rss_sample_interval_seconds'] = 0.05
    evidence['query_rss_scope'] = 'VmRSS samples between observed native measurement log markers; sampled, not exact peak'
