...
def draw(dataset):
    ...
    contract={'backend':'python','claim':'Compare static plus dynamic records to accepted historical controls at the same budget','archetype':'quantitative comparison','size_mm':[183,128],'query_count_per_L':800,'repeats':2,'aggregation':'total queries / total seconds','spread':'two-run min-max, not CI','exports':['png','pdf','svg'],'excluded_rows':0}
    (dest/'figure_contract.json').write_text(json.dumps(contract,indent=2))
    fig,ax=canvas(('GIST' if dataset=='gist' else 'AG News')+': Dynamic Records Comparison','800 test queries per L; two runs; controls reused from prior measurements')
    specs=[('Baseline',['baseline_start','baseline_end'],'#666666',':','o'),('Hot Records',['hot_records_r1','hot_records_r2'],'#C76A2C','-','s'),('Hot + Dynamic',['hot_dynamic_r1','hot_dynamic_r2'],'#379584','-','D'),(f'Hybrid {selected/100:g}%',['hybrid_r1','hybrid_r2'],'#1679B5','-','^')]
    raw=[];hashes={};ref=None
    ...
    fig.legend(*ax.get_legend_handles_labels(),loc='upper center',bbox_to_anchor=(.54,.85),ncol=2,frameon=False)
    ...
    (dest/'qa.json').write_text(json.dumps({'source_rows':320,'excluded':0,'source_hashes':hashes,'visual_review':'pending','notes':'Shared helpers export 183x128 mm, PNG 300 dpi and editable PDF/SVG; no TIFF requested.'},indent=2))
