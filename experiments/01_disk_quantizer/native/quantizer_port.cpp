...
#include "../tools/quantizer_kernels.cpp"
...
std::pair<double, uint64_t> compare_reference(
        const fs::path& path,
        const std::vector<SweepResult>& sweeps) {
    std::ifstream in(path, std::ios::binary);
    ...
    for (const auto& sweep : sweeps) {
        ...
        if (!width || values % width) throw std::runtime_error("invalid parity dimensions");
        query_comparisons += values / width;
        std::vector<float> reference(values);
        in.read(reinterpret_cast<char*>(reference.data()), values * sizeof(float));
        for (size_t i = 0; i < reference.size(); ++i) {
            if (!std::isfinite(reference[i]) || !std::isfinite(sweep.distances[i]))
                throw std::runtime_error("nonfinite parity distance");
    ...
    return {delta, query_comparisons};
}
...
std::tuple<std::string, double, double, uint64_t> read_parity_status(const fs::path& path) {
    std::ifstream in(path);
    std::string hash;
    double distance = 0.0, recall = 0.0;
    uint64_t comparisons = 0;
    in >> hash >> distance >> recall >> comparisons;
    if (!in || hash.size() != 64 || !comparisons) {
        throw std::runtime_error("validated parity status is missing");
    }
    return {hash, distance, recall, comparisons};
}
...
void write_artifact(
    ...
    out << std::setprecision(12)
        ...
        << ", \"query_comparisons_scope\": \"validation_query_width_pairs\""
...
int run(int argc, char** argv) {
    ...
    if (phase == "validate" || phase == "validation") {
        if (!disk_mode) {
            ...
            for (const size_t width : widths) {
                disk_reference.push_back(run_sweep(
                    codec, index, true, {}, queries, candidates, order,
            ...
            write_parity_status(
                    parity_path, reference_hash, max_distance_delta, max_recall_delta, query_comparisons);
        }
    } else if (phase == "test") {
    ...
    write_artifact(
            ...
            reference_hash,
            max_distance_delta,
            max_recall_delta, query_comparisons);
    return 0;
}
