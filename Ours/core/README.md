...
integration bridge is `experiments/02_diskann_fair/native/rabitq_bridge.cpp`).
...
(`experiments/01_quantizer_fair/faiss_quantizer_smoke.cpp`) and the
...
* Experiment 01 includes `hnswlib/space_rabitq.h` with include root
  `Ours/core/` (see `experiments/01_quantizer_fair/CMakeLists.txt`).
* Experiment 02 compiles `rabitq_bridge.cpp` with `-I <repo_root>` and
  includes `Ours/core/hnswlib/...` (see `experiments/02_diskann_fair/build.rs`).
